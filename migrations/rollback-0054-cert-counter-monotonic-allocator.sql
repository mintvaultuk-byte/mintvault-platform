...
--
-- ⚠️ APPLYING THIS RE-OPENS THE DEFECT 0054 CLOSED. Afterwards any role holding
...
-- re-apply 0054 immediately afterwards.
--
-- NOT DESTRUCTIVE TO DATA. The allocator TABLE and its row are deliberately left alone: 0054 only
...
--
-- 0052's column grants are likewise NOT reverted here. 0054 re-asserted them rather than authoring
...
-- scripts/db/migrate.ts:planMigrations (migrate.ts:449-462) would still classify 0054 as
...
BEGIN
  IF to_regclass('public.cert_counter') IS NULL THEN
    RAISE NOTICE 'rollback-0054: cert_counter absent; nothing to do.';
    RETURN;
  END IF;
...
BEGIN
  ...
  IF ntrg <> 0 THEN
    RAISE EXCEPTION 'rollback-0054: % cert_counter guard trigger(s) survive', ntrg;
  END IF;
  IF to_regprocedure('public.cert_counter_monotonic_guard()') IS NOT NULL THEN
    RAISE EXCEPTION 'rollback-0054: cert_counter_monotonic_guard() survives';
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_class WHERE oid = 'public.cert_counter'::regclass) THEN
    RAISE EXCEPTION 'rollback-0054: cert_counter was dropped; this file must never remove the allocator';
  END IF;
END$$;

-- De-journal, so the runner re-applies 0054 on its next run instead of skipping it as applied.
...
BEGIN
  IF to_regclass('public.schema_migrations') IS NOT NULL THEN
    EXECUTE $q$DELETE FROM schema_migrations
                WHERE filename = '0054_cert_counter_monotonic_allocator.sql'$q$;
  END IF;
END$$;
