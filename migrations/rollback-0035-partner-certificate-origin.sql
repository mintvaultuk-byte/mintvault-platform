...
BEGIN
  IF to_regclass('public.schema_migrations') IS NOT NULL THEN
    ...
    IF to_regclass('public.schema_migration_rollbacks') IS NOT NULL THEN
      ...
      SELECT m.filename, m.checksum, COALESCE((SELECT MAX((regexp_match(filename,'^([0-9]{4,})_'))[1]::int) FROM schema_migrations),0), 'rollback-0035-partner-certificate-origin.sql'
        FROM schema_migrations m
       WHERE filename = '0035_partner_certificate_origin.sql'
