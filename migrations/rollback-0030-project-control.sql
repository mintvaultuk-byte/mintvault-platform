...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables
    ...
    IF to_regclass('public.schema_migration_rollbacks') IS NOT NULL THEN
      ...
      SELECT m.filename, m.checksum, COALESCE((SELECT MAX((regexp_match(filename,'^([0-9]{4,})_'))[1]::int) FROM schema_migrations),0), 'rollback-0030-project-control.sql'
        FROM schema_migrations m
       WHERE filename = '0030_project_control.sql'
