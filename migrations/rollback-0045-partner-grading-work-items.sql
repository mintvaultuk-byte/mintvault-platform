...
--
-- D1 (critical) — the guard was BLIND. 0045 sets FORCE ROW LEVEL SECURITY, which applies the tenant
...
--
-- D2 (high) — uq_partner_grading_work_items_certificate_scope, created by 0045 on the PROTECTED
--   certificates table, was missing from the drop list and survived a "clean" rollback.
--
-- D3 (high) — 0045 grants SELECT/INSERT/UPDATE on cert_counter and USAGE/SELECT on
--   certificates_id_seq to partner_connector_runtime. Neither was revoked, so a rolled-back 0045
...
BEGIN
  ...
  IF to_regclass('public.schema_migrations') IS NOT NULL THEN
    EXECUTE $q$SELECT count(*) FROM schema_migrations
                WHERE filename ~ '^[0-9]{4}_' AND left(filename,4)::integer > 45$q$
      INTO later_migrations;
    IF later_migrations > 0 THEN
      RAISE EXCEPTION 'rollback-0045 refused: % later migration journal row(s) exist. Resolve newer migrations first.',
        later_migrations;
    END IF;
  END IF;
  ...
  IF NOT COALESCE((SELECT rolbypassrls FROM pg_roles WHERE rolname = current_user), false) THEN
    IF pg_get_userbyid((SELECT relowner FROM pg_class
      ...
      RAISE EXCEPTION
        'rollback-0045 refused: executor % neither owns partner_grading_work_items nor holds BYPASSRLS, so the evidence guard cannot observe every row.',
        current_user;
    END IF;
  ...
  IF NOT COALESCE((SELECT rolbypassrls FROM pg_roles WHERE rolname = current_user), false)
    ...
    RAISE EXCEPTION
      'rollback-0045 refused: FORCE ROW LEVEL SECURITY is still in effect for executor %; the evidence guard would under-count.',
      current_user;
  END IF;
  ...
  IF linked > 0 OR advanced > 0 OR assigned > 0 THEN
    ...
    RAISE EXCEPTION
      'rollback-0045 refused: live grading evidence present in % of % partner_grading_work_items row(s) (certificate_linked=%, status_advanced=%, grader_assigned=%). First offending work item id(s): %. Preserve provenance or perform a supervised data migration first.',
...
-- D2: 0045 creates this on the PROTECTED certificates table. Must come after DROP TABLE, because
...
-- repeated rollback still revokes. 0045 is the ONLY migration granting any of these three objects
...
BEGIN
  IF to_regclass('public.schema_migrations') IS NOT NULL THEN
    EXECUTE $q$DELETE FROM schema_migrations WHERE filename = '0045_partner_grading_work_items.sql'$q$;
  END IF;
END$$;
