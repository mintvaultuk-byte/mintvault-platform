-- ROLLBACK for 0046_partner_connector_profile_read.sql.
--
...
--
-- WHAT IT UNDOES: the single column-level SELECT grant 0046 issued. Nothing else — 0046 creates no
...
BEGIN
  IF to_regclass('public.partner_profiles') IS NULL THEN
    RAISE NOTICE 'rollback-0046: partner_profiles absent — nothing to revoke.';
    RETURN;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'partner_connector_runtime') THEN
    RAISE NOTICE 'rollback-0046: partner_connector_runtime absent — nothing to revoke.';
    RETURN;
  END IF;
  ...
  IF has_column_privilege('partner_connector_runtime', 'public.partner_profiles', 'trading_name', 'SELECT') THEN
    RAISE EXCEPTION
      'rollback-0046 assertion failed: partner_connector_runtime can still SELECT partner_profiles.trading_name (a table-level grant from elsewhere may be in play).';
  END IF;

  RAISE NOTICE 'rollback-0046: revoked partner_connector_runtime SELECT on partner_profiles(tenant_id, trading_name).';
END$$;
