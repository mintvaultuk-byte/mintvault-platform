-- COMPREHENSIVE ROLLBACK for Partner Network Phase 1 (migrations 0001–0005).
-- NOT a forward migration (no NNNN_ prefix → the runner ignores it). Destructive (drops the whole
-- partner_* family + helper functions + restricted role). Owner-approved protected action only;
-- rehearse on a disposable DB first. Touches NO existing MintVault table — existing data is preserved.
...
-- restricted role (after its objects are gone)
DO $$
BEGIN
