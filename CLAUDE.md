...
Browser → Express (server/index.ts)
  → Mounted domain routers + remaining server/routes.ts handlers
    → Domain services/repositories (including legacy server/storage.ts)
      → PostgreSQL / provider ports under each domain authority
...
- `shared/` — Canonical shared contracts; `shared/schema.ts` is the core Drizzle schema, with domain-specific contracts in their existing modules. Do not duplicate authority.
- `migrations/` — Numbered SQL migration history consumed by the canonical migration runner; `db:push` is not a replacement for that history. Preserve separate existing VQ lineage pending its approved convergence repair.
...
Drizzle ORM with PostgreSQL. Core legacy storage uses `server/storage.ts`; Partner,
Vault Quest and other bounded services already own domain repositories/queries. Follow
[architecture authority](docs/engineering/ARCHITECTURE_AUTHORITY.md), not an all-in-one
IStorage prescription. Preserve canonical shared types using `$inferSelect`/`$inferInsert`.
...
4. Verify startup through the approved disposable test harness; do not use `npm run dev` during remediation because it loads the local `.env`.

### Coding standards

- Reuse canonical shared/domain types and schemas; never duplicate authoritative definitions
- Put queries in their existing bounded-context repository/service; do not enlarge IStorage by default
- Put API handlers in their domain routers and prove the real root mount; avoid new root back-imports
...
1. Identify the existing domain owner and canonical shared/schema contract
2. Add behavior to its repository/service with reviewed migration authority if needed
3. Add domain API routes and verify composition, permissions and client contracts
4. Add UI components to `client/src/`
5. Run `npm run check` to verify types
6. Test through a process-isolated disposable environment without loading live `.env`
...
historically acted as the "Lead Engineer". Under the current Engineering OS, Codex is
implementation lead and Claude Opus High is the independent hostile reviewer. The active
Lead is the only agent allowed to adjudicate scope and integrate
edits; owner authority still controls risky actions (migrations, deploys, secrets, infra). If
...
For the next repository remediation/Graph Loop run, read the current entry point in
...
and `hygiene-dispatch.json` beside it. Run `validate-hygiene-dispatch.py`, dispatch its
bounded read-only lanes, and require `--dispatched` plus Lead adjudication before product
repair. Retain existing graph/issue IDs and owner approvals. This is sequencing authority,
with the completed Sep5 dispatch retained at `80a20611` in the wave proof record. Later
checkpoints require dependency-specific evidence refresh, not a fresh nine-lane audit or
relabeling old reports as new-SHA proof. The unavailable security lane remains UNKNOWN.
This is
not deployment, shared-migration, secret, GitHub-mutation or deletion permission; it does
not restart completed investigations or expand the broad hostile-review allowance.
