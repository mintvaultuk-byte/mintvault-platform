...
function configureRuntime({ isPackaged, resourcesPath, execPath }) {
...
function verifyHelperAt({ helperPath, manifestPath, runtime, runTool = defaultRunTool }) {
  ...
  if (runtime.isPackaged) {
    const expectedHelper = path.join(runtime.resourcesPath, "helpers", HELPER_FILENAME);
    const expectedManifest = path.join(runtime.resourcesPath, "helpers", MANIFEST_FILENAME);
...
module.exports = {
  ...
  _private: { verifyHelperAt, appBundleFromExecutable, readManifest },
};
