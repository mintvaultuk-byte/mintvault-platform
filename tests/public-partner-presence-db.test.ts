...
describe("0101 public Partner privacy/publication authority (real PostgreSQL 17)", () => {
  ...
  beforeAll(async () => {
    ...
    await admin.connect();
    await applyMigrationsRealistic(admin, cluster.url, [...PARTNER_MIGRATIONS, "0101_partner_public_presence"]);
    await admin.query(`
  ...
  it("rolls the public schema back without touching operational Partner data", async () => {
    ...
    const rollback = readFileSync(join(process.cwd(), "migrations/rollback-0101-partner-public-presence.sql"), "utf8");
