...
describe("0103 Partner Google presence (real PostgreSQL 17)", () => {
  ...
  beforeAll(async () => {
    ...
    await admin.connect();
    await applyMigrationsRealistic(admin, cluster.url, [...PARTNER_MIGRATIONS, "0102_partner_public_presence", "0103_partner_google_presence"]);
    await admin.query(`
  ...
  it("rolls 0102 back cleanly in the disposable database", async () => {
    const rollback = readFileSync(join(process.cwd(), "migrations/rollback-0103-partner-google-presence.sql"), "utf8");
