...
describe("Command Centre station readability probe", () => {
  it("fails closed when the cross-tenant role is blocked by FORCE RLS", async () => {
    partnerAdminQuery.mockImplementation((query: string) => Promise.resolve(query.includes("pg_roles")
      ...
      : { rows: [{ relname: "partner_stations", relkind: "r", relrowsecurity: true, relforcerowsecurity: true, owner: "owner_role", profile_revision_schema: true }] }));
  ...
  it("permits a station aggregate only for an RLS-capable read role", async () => {
    partnerAdminQuery.mockImplementation((query: string) => Promise.resolve(query.includes("pg_roles")
      ? { rows: [{ role_name: "app_role", rolsuper: false, rolbypassrls: true }] }
      : { rows: [{ relname: "partner_stations", relkind: "r", relrowsecurity: true, relforcerowsecurity: true, owner: "owner_role", profile_revision_schema: true }] }));
    await expect(getPartnerStationReadVisibility()).resolves.toEqual({ ok: true, profileRevisionSchema: true });
  });

  it("reports an absent optional profile-revision column without failing station visibility", async () => {
    partnerAdminQuery.mockImplementation((query: string) => Promise.resolve(query.includes("pg_roles")
      ? { rows: [{ role_name: "app_role", rolsuper: false, rolbypassrls: true }] }
      : { rows: [{ relname: "partner_stations", relkind: "r", relrowsecurity: true, relforcerowsecurity: true, owner: "owner_role", profile_revision_schema: false }] }));
    await expect(getPartnerStationReadVisibility()).resolves.toEqual({ ok: true, profileRevisionSchema: false });
  });
});
