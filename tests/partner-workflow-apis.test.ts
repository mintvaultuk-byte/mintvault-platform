...
(isLocal ? describe : describe.skip)("Partner Phase 2 completion — new workflow APIs (disposable DB, real HTTP)", () => {
  beforeAll(async () => {
  ...
  describe("customers", () => {
    ...
    it("script-shaped and SQL-shaped values are stored as inert text, never executed", async () => {
      ...
      const sqlish = await j("POST", "/api/partner/customers", cookie, { fullName: "Robert'); DROP TABLE partner_customers;--" });
  ...
  describe("PATCH /submissions/:id/cards/:cardId", () => {
    it("a valid edit succeeds and writes a card_updated event", async () => {
      ...
      const edited = await j("PATCH", `/api/partner/submissions/${id}/cards/${card.body.id}`, cookie, { cardName: "Edited Name", year: 1999 });
    ...
    it("cross-tenant card edit is denied (404, no existence leak)", async () => {
      ...
      const attempt = await j("PATCH", `/api/partner/submissions/${idA}/cards/${cardA.body.id}`, cookieB, { cardName: "Hijacked" });
      expect(attempt.status).toBe(404);
      const unchanged = await admin.query("SELECT card_name FROM partner_submission_cards WHERE id=$1", [cardA.body.id]);
    ...
    it("cross-location card edit is denied for a location-scoped user", async () => {
      ...
      const attempt = await j("PATCH", `/api/partner/submissions/${idL2}/cards/${card.body.id}`, receptionCookie, { cardName: "Hijacked" });
      expect(attempt.status).toBe(404);
    });
    ...
    it("a card ID belonging to a DIFFERENT submission (same tenant) is rejected", async () => {
      ...
      const attempt = await j("PATCH", `/api/partner/submissions/${id2}/cards/${card.body.id}`, cookie, { cardName: "Wrong Submission" });
      expect(attempt.status).toBe(404);
    });
    ...
    it("editing a card on an already-submitted submission is denied", async () => {
      ...
      const submitted = await j("POST", `/api/partner/submissions/${id}/submit`, cookie, { idempotencyKey: "card-edit-lock-" + id });
      expect(submitted.status).toBe(200);
      const attempt = await j("PATCH", `/api/partner/submissions/${id}/cards/${card.body.id}`, cookie, { cardName: "Too Late" });
    ...
    it("invalid quantity (zero, negative, non-integer) is rejected, existing quantity untouched", async () => {
      const cookie = await login("owner@apia.com");
      const id = await draftId(cookie);
      const card = await j("POST", `/api/partner/submissions/${id}/cards`, cookie, { cardName: "Qty Card", quantity: 3 });
      ...
      const unchanged = await admin.query("SELECT quantity FROM partner_submission_cards WHERE id=$1", [card.body.id]);
    ...
    it("invalid year is rejected rather than silently no-op'd", async () => {
      const cookie = await login("owner@apia.com");
      const id = await draftId(cookie);
      const card = await j("POST", `/api/partner/submissions/${id}/cards`, cookie, { cardName: "Year Card", year: 2000 });
    ...
    it("invalid declared value is rejected rather than silently no-op'd", async () => {
      const cookie = await login("owner@apia.com");
      const id = await draftId(cookie);
      const card = await j("POST", `/api/partner/submissions/${id}/cards`, cookie, { cardName: "Value Card", declaredValuePence: 500 });
      const r = await j("PATCH", `/api/partner/submissions/${id}/cards/${card.body.id}`, cookie, { declaredValuePence: -1 });
      expect(r.status).toBe(400);
      expect(r.body.error.code).toBe("invalid_declared_value");
      const unchanged = await admin.query("SELECT declared_value_pence FROM partner_submission_cards WHERE id=$1", [card.body.id]);
    ...
    it("oversized text field is rejected", async () => {
      ...
      const r = await j("PATCH", `/api/partner/submissions/${id}/cards/${card.body.id}`, cookie, { customerNotes: "x".repeat(600) });
      expect(r.status).toBe(400);
    });
    ...
    it("unknown card ID is rejected as not-found", async () => {
      const cookie = await login("owner@apia.com");
      const id = await draftId(cookie);
      const r = await j("PATCH", `/api/partner/submissions/${id}/cards/00000000-0000-0000-0000-000000000000`, cookie, { cardName: "Ghost" });
      expect(r.status).toBe(404);
    });
