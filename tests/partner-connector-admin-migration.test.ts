...
(isLocal ? describe : describe.skip)("Trusted Intake Connector G4 — migration 0014 admin-actions audit (disposable DB)", () => {
  beforeAll(async () => {
    ...
    await admin.query("CREATE TABLE IF NOT EXISTS submissions (id serial primary key, user_id varchar not null, tracking_number text not null unique)");
  ...
  it("0014 is applied and journaled", async () => {
    const { rows } = await admin.query("SELECT status FROM schema_migrations WHERE filename = '0014_partner_connector_admin_actions.sql'");
  ...
  it("only the two evidence-backed indexes (+ idempotency unique + pkey) exist on the table", async () => {
    const { rows } = await admin.query("SELECT indexname FROM pg_indexes WHERE tablename = $1 ORDER BY indexname", [TABLE]);
    const names = rows.map((r) => r.indexname);
    expect(names).toEqual([
      "idx_partner_connector_admin_actions_org",
  ...
  it("G4 rollback drops exactly the new table + removes the journal row + reapplies cleanly", async () => {
    ...
    try {
      ...
      const j = await admin.query("SELECT 1 FROM schema_migrations WHERE filename='0014_partner_connector_admin_actions.sql'");
