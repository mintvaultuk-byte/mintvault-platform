...
describe("Partner MFA enrolment (real render)", () => {
  ...
  it("restart setup invalidates the displayed setup and confirms against the fresh enrolment", async () => {
    apiRequest.mockImplementation((method: string, url: string) => {
      if (url.endsWith("/mfa/enrol"))
        return ok({
          ok: true,
  ...
  it("cancel setup redirects only after the server confirms cancellation", async () => {
    ...
    apiRequest.mockImplementation((method: string, url: string) => {
      ...
      if (url.endsWith("/mfa/cancel")) return fail(503, "unavailable");
      return ok({ ok: true });
    });
    try {
      await mountEnrolment();
      ...
      expect((await waitForTestId("text-mfa-enrol-error")).textContent).toContain("could not cancel");

      apiRequest.mockImplementation((method: string, url: string) => {
        if (url.endsWith("/mfa/cancel")) return ok({ ok: true });
        return ok({ ok: true });
      });
      await clickTestId("button-mfa-enrol-sign-out");
