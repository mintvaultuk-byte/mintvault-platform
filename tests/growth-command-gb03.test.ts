...
const validApplication: PartnerApplicationInput = {
  ...
  interestReason:
    "We would like to understand whether a local MintVault grading route could suit our collector community.",
  physicalRetail: true,
...
describe("GB-03 public Partner acquisition contract", () => {
  it("renders a premium public application page with the factual operational boundary", () => {
    const markup = renderToStaticMarkup(
      createElement(
        ...
        { value: { legalPagesLive: true, privacyNoticeLive: true, partnerApplicationsLive: true } },
        createElement(PartnersPage)
      )
    );
  ...
  it("keeps capture unavailable when the legal publication flag is off", () => {
    const markup = renderToStaticMarkup(
      createElement(
        FeatureFlagsContext.Provider,
        { value: { legalPagesLive: false, privacyNoticeLive: false, partnerApplicationsLive: false } },
        createElement(PartnersPage)
      )
    );
  ...
  it("uses one dual publication gate for the application, SSR and legal notice", () => {
    expect(arePartnerApplicationsLive({ PRIVACY_NOTICE_LIVE: "false", PARTNER_APPLICATIONS_LIVE: "false" })).toBe(
      false
    );
    ...
    expect(
      isLegalDocumentPublic("privacy-policy", {
        LEGAL_PAGES_LIVE: false,
        PRIVACY_NOTICE_LIVE: true,
        PARTNER_APPLICATIONS_LIVE: false,
      })
    ).toBe(true);
    expect(
      isLegalDocumentPublic("website-terms", {
        LEGAL_PAGES_LIVE: false,
        PRIVACY_NOTICE_LIVE: true,
        PARTNER_APPLICATIONS_LIVE: false,
      })
  ...
  it("publishes a factual, versioned Partner privacy notice without draft or bundled-marketing language", () => {
    ...
    expect(DOCKERFILE).toContain("COPY --from=production-dependencies /app/content/legal ./content/legal");
  });
  ...
  it("validates the short application server-side and rejects malformed or unapproved input", () => {
    ...
    expect(
      partnerApplicationSchema.safeParse({ ...validApplication, webPresence: "javascript:alert(1)" }).success
    ).toBe(false);
    expect(
      partnerApplicationSchema.safeParse({ ...validApplication, webPresence: "https://user:pass@shop.example" }).success
    ).toBe(false);
    expect(partnerApplicationSchema.safeParse({ ...validApplication, postcode: "not-a-postcode" }).success).toBe(false);
    expect(partnerApplicationSchema.safeParse({ ...validApplication, interestReason: "too short" }).success).toBe(
      false
    );
    expect(partnerApplicationSchema.safeParse({ ...validApplication, privacyAcknowledged: false }).success).toBe(false);
    expect(partnerApplicationSchema.safeParse({ ...validApplication, internalStatus: "QUALIFIED" }).success).toBe(
      false
    );
  });
    ...
    const duplicate = await persistPartnerApplication(
      {
        transaction: async (operation) =>
          operation(async (query) => {
            ...
            return duplicateCalls.length === 1
              ? { rows: [] }
  ...
  it("keeps application responses out of logs and makes notification non-authoritative", () => {
    ...
    expect(PUBLIC_ROUTES.indexOf("persistPartnerApplication")).toBeLessThan(
      PUBLIC_ROUTES.indexOf("sendPartnerApplicationNotification")
    );
