...
import { shouldRateLimitAdminCredentialRequest } from "../server/lib/admin-auth-session";
...
describe("admin auth reliability", () => {
  it("does not rate-limit harmless admin session refresh checks", async () => {
    const limiter = rateLimit({ windowMs: 60_000, max: 5, validate: false });
    const app = express();
    app.use("/api/admin/session", (req, res, next) => {
      if (!shouldRateLimitAdminCredentialRequest(req)) return next();
