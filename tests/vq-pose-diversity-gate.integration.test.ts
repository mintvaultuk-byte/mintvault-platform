...
// A real solid-white 200x200 PNG — action_pose tests never check background so TINY_PNG
// is fine there, but the Master Reference path runs the REAL validateStudioBackground
// (not mocked), which rejects TINY_PNG's black pixel outright. Only used by the Master
// test below via a one-off createSpy override.
const WHITE_PNG = vi.hoisted(() => Buffer.from(
...
const createSpy = vi.hoisted(() => vi.fn(async () => ({
  provider: "higgsfield" as const, model: "nano_banana", png: TINY_PNG, width: 512, height: 512, jobId: "job-pose-gate",
})));
...
const CHARACTER = vi.hoisted(() => ({
  ...
  characterName: "Pose Gate Subject", element: "Flame", bodyShape: "small", colours: "red/orange", markings: "flame tip tail", eyes: "amber", tailAccessories: "none",
  locked: false,
}));
...
run("Action Reference pose-diversity gate — route wiring", () => {
  ...
  beforeEach(async () => {
    createSpy.mockReset();
    createSpy.mockResolvedValue({ provider: "higgsfield" as const, model: "nano_banana", png: TINY_PNG, width: 512, height: 512, jobId: "job-pose-gate" });
    scoreSpy.mockReset();
  ...
  it("pose FAILS on attempt 1, PASSES on the automatic retry — accepted, exactly 2 provider calls", async () => {
    ...
    const res = await post(`/api/admin/vault-quest/characters/GNV-F01-S1/generate-artwork`, { referenceType: "action_pose", model: "nano_banana", idempotencyKey: `pose-retry-ok-${keyCounter}` });
  ...
  it("pose FAILS on BOTH attempts — capped at 2 provider calls, stays VISIBLE (not auto-rejected), cannot be approved", async () => {
    ...
    const res = await post(`/api/admin/vault-quest/characters/GNV-F01-S1/generate-artwork`, { referenceType: "action_pose", model: "nano_banana", idempotencyKey: `pose-both-fail-${keyCounter}` });
    ...
    const approve = await post(`/api/admin/vault-quest/characters/GNV-F01-S1/approve-candidate`, { candidateId: body.candidateId });
  ...
  it("a genuinely dynamic pose passes immediately — only 1 provider call, no wasted retry", async () => {
    ...
    const res = await post(`/api/admin/vault-quest/characters/GNV-F01-S1/generate-artwork`, { referenceType: "action_pose", model: "nano_banana", idempotencyKey: `pose-dynamic-${keyCounter}` });
    expect(res.status).toBe(201);
  ...
  it("SPEND ESTIMATE prices the WORST CASE (2 calls) up front, but the ACTUAL charge reflects only what really happened (1 call)", async () => {
    ...
    const res = await post(`/api/admin/vault-quest/characters/GNV-F01-S1/generate-artwork`, { referenceType: "action_pose", model: "nano_banana", idempotencyKey });
    expect(res.status).toBe(201);
  ...
  it("a reference type with NO retry path (e.g. no approved Master yet) is priced at the normal 1x rate, not doubled", async () => {
    ...
    const res = await post(`/api/admin/vault-quest/characters/GNV-F01-S1/generate-artwork`, { referenceType: "face_closeup", model: "nano_banana", idempotencyKey });
    expect(res.status).toBe(201);
  ...
  it("identity FAILS while pose PASSES — existing identity gate unaffected: still auto-rejected and hidden", async () => {
    scoreSpy.mockResolvedValueOnce({
      score: 40, verdict: "reject", threshold: 70,
    ...
    const res = await post(`/api/admin/vault-quest/characters/GNV-F01-S1/generate-artwork`, { referenceType: "action_pose", model: "nano_banana", idempotencyKey: `identity-fail-${keyCounter}` });
  ...
  it("Master Reference generation never invokes the pose gate — scoreCharacterIdentity called with NO masterPng", async () => {
    createSpy.mockResolvedValue({ provider: "higgsfield" as const, model: "nano_banana", png: WHITE_PNG, width: 200, height: 200, jobId: "job-master" }); // must pass the REAL background check
    scoreSpy.mockResolvedValue(identityPass()); // used by the post-upload scoreAndGateCandidate path, not the pose retry loop
    const res = await post(`/api/admin/vault-quest/characters/GNV-F01-S1/generate-artwork`, { referenceType: "master_portrait", model: "nano_banana", idempotencyKey: `master-unaffected-${keyCounter}` });
    ...
    createSpy.mockResolvedValue({ provider: "higgsfield" as const, model: "nano_banana", png: TINY_PNG, width: 512, height: 512, jobId: "job-pose-gate" }); // restore default for isolation
  });
});
