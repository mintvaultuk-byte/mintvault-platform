...
async function seedReady(claimant: string): Promise<{
  ...
  const cardId = uuid(base + 400_000);
  await admin.query(
    `INSERT INTO partner_submission_cards
     ...
     VALUES ($1,$2,$3,1,'Card 1',50000,1,$4,$5)`,
    [
      cardId,
      A,
      subId,
...
(isLocal ? describe : describe.skip)("Trusted Intake Connector G3F — blocker remediation proofs", () => {
  ...
  describe("3B — resumed import uses the newer fingerprint; the interrupted reservation's stale fingerprint is preserved", () => {
    it("end-to-end: reservation holds fp A (immutable); completed attempt holds the newer authorising run/fp; retry converges; no side effects", async () => {
      ...
      // No forbidden side effects outside the intended partner certificate materialisation.
      expect((await admin.query<{ n: number }>("SELECT count(*)::int AS n FROM certificates")).rows[0].n).toBe(2);
      expect(
        (await admin.query<{ n: number }>("SELECT count(*)::int AS n FROM certificates WHERE origin_type='PARTNER'"))
          .rows[0].n
      ).toBe(1);
