> **Last verified relevant:** 2026-05-05. Re-restored to HEAD after regression investigation found PIN auth (commit `9cc6fca`) regressed the unification — PIN login at `/api/auth/pin/login` was modeled after the pre-unification pattern (clear `userId` when setting `customerEmail`), defeating the design goal documented below. Fix shipped 2026-05-05 (PIN login + password login both now set both audiences). This doc remains the canonical reference for the session-key shape across all login entry points.
