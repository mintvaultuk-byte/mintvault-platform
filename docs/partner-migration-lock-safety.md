...
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------- |
| 0   | Dry-run (no `--apply`). Confirm the pending list is exactly **0047 → 0066** — twenty files, nothing else, `0 inconsistent`, `0 checksum-mismatch`. ⚠️ This said "exactly 0047-0052, six files" until 2026-08-09 and was **wrong and dangerous**: 0053-0066 exist on this branch, so an operator following it literally would either abort a legitimate run or, at step 5, apply fourteen unplanned files in one go — including two that take ACCESS EXCLUSIVE on `certificates`. | No — read-only                                                                   | none                                                                                                                                                                                                                   | n/a                       |
...
| 5   | **`--apply --to=0052`** → applies **0051** then **0052**. ⚠️ The `--to` is MANDATORY and was missing here until 2026-08-09; a bare `--apply` now runs everything through 0066.                                                                                                             | **YES.** See the 0051 warning below — this is the riskiest step in the set.      | 0051: 3 ms `ACCESS EXCLUSIVE` on `partner_feature_flags`, read on EVERY partner request. 0052: 9 ms `ACCESS EXCLUSIVE` on `partner_internal_notes`, `partner_management_audit`, `partner_service_tiers`.               | See below.                |
| 6   | Verify: journal shows 0047-0052 `applied`, `/api/version`, partner login, a public `/cert/{id}` lookup.                                                                | No                                                                               | none                                                                                                                                                                                                                   | n/a                       |
| 7   | **`--apply --to=0057`** → applies **0053** … **0057**                                                                                                                  | **YES — short.** 0054 blocks certificate ISSUANCE.                               | 0054: `ShareRowExclusive` on `cert_counter` — the allocator's `UPDATE` is blocked, so **no certificate can be issued** for the duration. Reads unaffected (measured; an earlier note said ACCESS EXCLUSIVE — wrong mode, right blast radius). 0053/0055/0056/0057: partner tables only. | See below.                |
| 8   | **`--apply --to=0062`** → applies **0058** … **0062**                                                                                                                  | **Prefer a low-WRITE window.**                                                   | 0058: `ShareLock` on `certificates`, held for the WHOLE 62 KB file (the runner holds a file's locks until COMMIT), so certificate **writes** are blocked for 0058's full execution. **Reads keep serving** — the public verification page stays up. 0059-0062: partner tables only. | See below.                |
| 9   | **`--apply --to=0064`** → applies **0063** then **0064**                                                                                                               | **YES — and this is the only step that can 404 the public certificate page.**    | Both take **ACCESS EXCLUSIVE** on `certificates`, which blocks **reads as well as writes**. Both are catalog-only (nullable columns, no DEFAULT → no table rewrite) and each carries its own `SET LOCAL lock_timeout = '2s'`, so the exposure is the lock QUEUE, not the work. Sub-second once granted. | Abort + retry when quiet. |
| 10  | **`--apply --to=0065`** → applies **0065**                                                                                                                             | **No.**                                                                          | `CREATE INDEX CONCURRENTLY` — `ShareUpdateExclusive` only, which conflicts with nothing ordinary traffic takes. Runs OUTSIDE a transaction (`migrate:no-transaction`) and is the only such file besides 0018. Takes minutes on a large `certificates`; that is expected and blocks nothing. | Re-run; self-heals.       |
| 11  | **`--apply`** → applies **0066**                                                                                                                                       | **No.**                                                                          | `partner_public_listings` only — zero rows on staging and production until 0058 lands a listing. Does not touch `certificates`, `cert_counter` or `submissions`.                                                       | See below.                |
| 12  | Verify: journal shows all twenty `applied`, `/api/version`, partner login, a public `/cert/{id}` lookup, and a public slab image.                                       | No                                                                               | none                                                                                                                                                                                                                   | n/a                       |
...
**Step 9 deserves its own sentence, because it is the only one in the whole series that can take
the public trust surface down.** Every other blocking step leaves `certificates` READABLE — 0058
blocks writes, 0054 blocks the counter, and the public `/cert/{id}` page keeps serving throughout.
0063 and 0064 take ACCESS EXCLUSIVE, so for the moment they hold it a certificate lookup returns
nothing. The work is microseconds; the risk is entirely in waiting for the lock behind a long
transaction, which the in-file 2 s `lock_timeout` bounds and converts into a clean abort.

**⚠️ DEPLOY PREREQUISITE, and it is not a migration.** After 0064 the anonymous slab-image proxy
reads `public_slab_image_projection` as `partner_public_reader` and **fails closed** — it will not
fall back to the privileged pool. Before the APPLICATION deploy that carries this code:

  1. `PARTNER_PUBLIC_DATABASE_URL` must be set to a login role's connection string; and
  2. that login role must be a member of the group role, which no migration can do:
     `GRANT partner_public_reader TO <login_role>;`

Migration 0061 creates `partner_public_reader` as NOLOGIN per the house convention (0008:17-18)
that infrastructure grants membership out of band. **The public slab showcase is a LIVE production
surface today** and every image on it 503s without both of the above. The startup probe
(`reportPartnerPublicNetworkReadiness`) logs `public_reader_role_unavailable` with the exact remedy
if the grant is missing, and `/api/shops` stays dark regardless because its flag defaults OFF.

Steps 1, 3, 5, 7, 8, 9, 10 and 11 touch the database. Steps 0, 2, 4, 6 and 12 are verification.
