...
**Critical Partner gate:** 35 suites / 685 assertions / 0 failed / **0 skipped**.
...
| **AT-21** | Webhook grant under concurrent NEW                                | Grant idempotency **GREEN**; the _simultaneous grant-plus-NEW boundary_ is not exercised   | **PARTIAL** | `partner-credit-purchase.test.ts` + `L1`; see §Gaps                                                                |
...
**AT-21 — PARTIAL.** Webhook grant idempotency is proven (`partner-credit-purchase.test.ts`) and the
last-credit race is proven (`L1`). What is _not_ exercised is the two happening at the same instant:
a verified webhook granting 10 credits while two stations attempt NEW across the grant boundary. The
mechanism that would carry it is the same wallet row lock already proven in `L1`, and availability is
never cached in application code — but that is an argument, not a test, and it is recorded here as an
argument.
