...
- `tests/public-partner-presence-db.test.ts` applies public `0102` to disposable PostgreSQL 17, proves versioned consent/approval/privacy and rollback without operational-data loss.
- `tests/google-partner-presence-migration.test.ts` applies optional Google `0103`, exercises its constraints/RLS/service flow and proves its five Google tables plus Google-only session constraint roll back.
