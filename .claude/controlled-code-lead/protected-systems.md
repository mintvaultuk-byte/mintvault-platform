...
| Vault Quest DB | `shared/vq-schema.ts`, `drizzle-vq.config.ts`, `migrations-vq/` | Separate closed runner estate, journal and immutable SQL history; scoped Drizzle config is inspection-only | Never Drizzle push/generate or raw file apply; canonical `db:migrate -- --estate vault-quest`, dry-run before explicitly target-approved apply. Unjournalled history requires reviewed historical admission, not replay. See `docs/runbooks/db-migration-safety.md`. |
