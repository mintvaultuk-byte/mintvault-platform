...
| F2 | Imported uncaptured certificates are presented as Ready to Grade. | workflow/security + scanner review | blocker | confirmed | `grading-routes.ts:417-499` | B | yes | Local Proof | implemented / locally verified | Queue now requires two current terminal station-bound TIFF masters in the exact mapped location; focused boundary assertions and typecheck pass. |
