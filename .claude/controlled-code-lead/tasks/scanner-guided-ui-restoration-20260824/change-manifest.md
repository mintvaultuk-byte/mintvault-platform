...
| -------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | -------------- |
| `scripts/scanner-app/renderer/index.html`                | Wrap operational content in a hidden-by-default container; add a calibration-only health action.                                                                                                           | Prevent legacy capture controls from flashing or appearing before station authority is known.                                 | C              |
| `scripts/scanner-app/renderer/app.js`                    | Render operational work only for authoritative `active` + `VALID` calibration; initialise guided checking; retain setup recovery controls; hide legacy card/credit/diagnostic surfaces during calibration. | Enforce calibration/scanner health as the only operator path before card work.                                                | C              |
...
1. Hide all operational Scanner controls until authoritative setup returns `active` + `VALID` calibration.
2. Prevent the billing layer from rendering or polling before card work is authorised.
...
- [x] `npm run lint` — passed with pre-existing warnings and no errors.
- [x] `npm run build` — passed.
- [x] Fresh arm64 package plus package verifier — 1.5.4 passed.
- [x] Read-only local process/runtime-manifest inspection — sole 1.5.4 package declared STAGING, was physically observed in the guided calibration screen, then exited cleanly.
