...
**Authority:** the owner explicitly authorised GB-04B closeout, a dedicated read-only MCP identity/transport, a genuine review engine, privacy-safe public authority, conversion instrumentation and integration into the existing Growth Command. The Infrastructure Control / Cost / GBP addendum additionally authorised canonical GBP presentation, provider-read investigation, manual recommendation-only infrastructure/cost/readiness/incident/velocity contracts, and a future guarded-scaling design. The Commercial Scoreboard addendum authorises an auditable monthly target store, deterministic time-aware status, current-month Super Admin target editing, read-only MCP access and existing-command UI integration. This authorises local code, tests, additive migration authoring and documentation within those exact boundaries. It does **not** authorise applying a migration, writing a secret, pushing/merging, changing a provider account, letting AI/MCP change a target, mutating Fly/Neon/budget/capacity, or deploying before the separate release gates.
...
| H       | Make GBP canonical; add truthful Fly/Neon/cost/budget detail, deterministic readiness/incident priority, tiny-sample-safe verified revenue velocity and future guarded-scaling design; keep all infrastructure mutation unavailable               | Growth intelligence module/service, read-only MCP tools, Growth page, tests/docs                           |
| S       | Persist explicit current-month commercial targets set only by Super Admin; compare exact calendar-month actual progress with elapsed-period pace; render green/amber/red/grey status and traceable insights; expose read-only MCP scoreboard only | Existing `0101`, shared schema, scoreboard service, Growth aggregate/route/page, MCP read tool, tests/docs |

## Explicit exclusions

- No MVGS/grading, payment authority, Stripe fulfilment, Partner tenant/credits, Scanner/station, owner auth, secret value, provider account, outreach, infrastructure mutation, provider billing estimate, invented FX or automatic scaling changes.
- No public customer-level review data, arbitrary recipient/URL, review gating, grade/sentiment eligibility, fake provider metrics, inferred business target, rolling-30-day substitute for a calendar month, or request/click count presented as a genuine review.
...
- `growth_commercial_targets` is additive inside never-applied migration `0101`. It stores only controlled metric, monthly period start, positive target value, Super Admin actor and timestamps. The target write and generic audit record are one transaction; no payment amount or commercial actual is writable through this surface.
...
The addendum has a separate ceiling of **7 runtime/test files**, **1,000 net lines**, no migration, no dependency, no provider call and no infrastructure write. Any real provider adapter or privileged control path requires a new manifest and owner decision.

The Commercial Scoreboard addendum has a separate ceiling of **9 runtime/test files**, **1,400 net lines**, uses the existing `0101` identity, adds no dependency/provider call, and changes no payment/grading/auth/Partner/Scanner authority. Exceeding that ceiling or adding non-monthly target semantics requires re-freezing this manifest.
