...
const config: Record<CommandCentreKpiId, SourceConfig> = {
  ...
  "partner-onboarding-blocked": {
    source: "partner-operational-readiness",
    deepLink: "/admin/partners",
    freshnessSeconds: 300,
  },
  "partner-credit-projection": {
    source: "partner-wallet-projection",
    deepLink: "/admin/partners",
    freshnessSeconds: 60,
  },
  ...
  "connector-exception-count": {
    source: "partner-connector-operations",
    deepLink: "/admin/partners/infrastructure",
    freshnessSeconds: 300,
  },
  "non-terminal-submissions": {
    source: "submissions-operational-state",
    deepLink: "/admin?tab=submissions",
    freshnessSeconds: 60,
  },
  ...
  "grade-review-awaiting-decision": {
    source: "certificates-review-queue",
    deepLink: "/admin?tab=certs",
    freshnessSeconds: 60,
  },
  ...
  "ownership-transfer-exceptions": {
    source: "transfer-verifications",
    deepLink: "/admin?tab=transfers",
    freshnessSeconds: 300,
  },
  "paid-submissions-recorded": {
    source: "submissions-paid-recorded",
    deepLink: "/admin?tab=submissions",
    freshnessSeconds: 300,
  },
};
...
function success(
  ...
  const authoritativeZero =
    typeof value === "number"
      ? value === 0
      : Object.values(value).length > 0 && Object.values(value).every((entry) => entry === 0);
...
async function composeDashboard(period: CommandCentrePeriod): Promise<CommandCentreDashboardResponse> {
  ...
  if (partnerResult.status === "rejected" || !partnerResult.value.available) {
    ...
    if (
      ...
      !partnerResult.value.available &&
      partnerResult.value.visibilityFailure
    ) {
      attentionItems.push(
        makeAttention(
    ...
    kpis["station-lifecycle-state"] = partner.station
      ? success(
          "station-lifecycle-state",
          partner.station.PENDING + partner.station.ACTIVE + partner.station.SUSPENDED + partner.station.REVOKED === 0
            ? 0
            : partner.station,
          asOf
        )
      : failed("station-lifecycle-state", "UNAVAILABLE", partner.stationReasonCode ?? "PARTNER_STATION_UNAVAILABLE");
    kpis["connector-exception-count"] =
      partner.connectorCount === null
    ...
    for (const candidate of partner.onboardingBlocked ?? []) {
      attentionItems.push(
        makeAttention(
          "ATT-PARTNER-ONBOARDING-BLOCKED",
          candidate.id,
          "Partner onboarding blocked",
          "A canonical readiness gate says the Partner cannot operate.",
          "HIGH",
          "partner-operational-readiness",
          "/admin/partners/" + candidate.id + "/onboarding",
          candidate.timestamp || asOf,
          300
        )
      );
    }
    for (const candidate of partner.connectorCandidates) {
      attentionItems.push(
        makeAttention(
          "ATT-CONNECTOR-EXCEPTION",
          candidate.id,
          "Connector exception requires review",
          "A connector record needs a human in its canonical console.",
          "HIGH",
          "partner-connector-operations",
          "/admin/partners/infrastructure",
          candidate.timestamp || asOf,
          300
        )
      );
  ...
  if (coreResult.status === "rejected") {
    ...
    kpis["non-terminal-submissions"] =
      "value" in core.submissions
    ...
    kpis["paid-submissions-recorded"] =
      "value" in core.submissions
        ? core.submissions.value.paid.nonGbp > 0
          ? failed("paid-submissions-recorded", "UNKNOWN", "NON_GBP_PAYMENT_PRESENT")
          : success(
              "paid-submissions-recorded",
              { count: core.submissions.value.paid.count, amount: core.submissions.value.paid.amount },
              asOf
            )
    ...
    for (const candidate of "value" in core.scan ? core.scan.value.candidates : []) {
      attentionItems.push(
        makeAttention(
          "ATT-SCAN-UNASSIGNED",
          candidate.id,
          "Received submission awaits scan assignment",
          "A received, non-deleted submission still has no scan assignment.",
          "MEDIUM",
    ...
    for (const candidate of "value" in core.review ? core.review.value.candidates : []) {
      attentionItems.push(
        makeAttention(
          ...
          candidate.id,
          "Grade review awaits canonical decision",
          "A certificate has entered the existing review queue.",
          "HIGH",
    ...
    for (const candidate of "value" in core.print ? core.print.value.candidates : []) {
      attentionItems.push(
        makeAttention(
          ...
          candidate.id,
          "Print batch failed or partial",
          "A canonical print batch is failed or partially completed.",
          "HIGH",
          "print-batches",
          "/admin?tab=print-queue",
          candidate.timestamp || asOf,
          60
        )
      );
    ...
    for (const candidate of "value" in core.transfer ? core.transfer.value.candidates : []) {
      attentionItems.push(
        makeAttention(
          ...
          candidate.id,
          "Ownership transfer disputed",
          "An unresolved canonical transfer dispute requires use of the existing transfer workspace.",
          "HIGH",
  ...
  const attention = attentionItems
    ...
    .slice(0, 8);
  const partialSourceIds = COMMAND_CENTRE_KPI_IDS.map((id) => kpis[id])
...
export function composeCommandCentreDashboard(period: CommandCentrePeriod): Promise<CommandCentreDashboardResponse> {
