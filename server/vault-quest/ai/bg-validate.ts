/**
 * Studio-background validation for the MASTER REFERENCE (permanent canonical art).
 *
 * The Master Reference must sit on a plain approved studio backdrop (pure white,
 * very light cream, or very light neutral grey — a single flat colour). This runs
 * AFTER each generation: it samples the outer edge band (the region outside the
 * centred creature) and fails the image if that region isn't a plain approved
 * colour, or if more than 5% of it deviates (scenery / texture / gradient / flames
 * / coloured backdrop / vignette). Deterministic + free — no model call.
 *
...
export async function validateStudioBackground(png: Buffer): Promise<BgValidation> {
  ...
  const bg: [number, number, number] = [median(anchors.map((c) => c[0])), median(anchors.map((c) => c[1])), median(anchors.map((c) => c[2]))];
  ...
  const minC = Math.min(...bg), maxC = Math.max(...bg);
  const blueDominant = bg[2] > bg[0] + 8;
  if (minC < 200 || maxC - minC > 42 || blueDominant) {
    return { ok: false, offBackgroundFraction: 1, backgroundColor: bg, reason: `backdrop is not a plain white/cream/light-grey studio colour (rgb ${bg.join(",")})` };
  }
  ...
  return {
    ...
    backgroundColor: bg,
    reason: frac > OFF_BG_THRESHOLD ? `${(frac * 100).toFixed(1)}% of the edge is off-background (scenery / texture / gradient / effect)` : undefined,
  };
}
