...
async function adminAudit(tenantId: string, action: string, reason: string | null, adminEmail: string, recordType?: string, recordId?: string) {
...
export function superAdminPartnerRouter(): Router {
  ...
  r.get("/", async (_req, res) => {
    const { rows } = await partnerAdminQuery("SELECT id, legal_name, status, accreditation_level, health, created_at FROM partner_organisations ORDER BY created_at DESC");
    res.json(rows);
  });

  r.get("/:partnerId", async (req, res) => {
    const { rows } = await partnerAdminQuery("SELECT id, legal_name, status, accreditation_level, health FROM partner_organisations WHERE id=$1", [req.params.partnerId]);
    if (rows.length !== 1) {
  ...
  r.get("/:partnerId/locations", async (req, res) => {
    const { rows } = await partnerAdminQuery("SELECT id, name, status FROM partner_locations WHERE tenant_id=$1 ORDER BY name", [req.params.partnerId]);
    res.json(rows);
  });

  r.get("/:partnerId/users", async (req, res) => {
    const { rows } = await partnerAdminQuery("SELECT id, email, status, mfa_enabled, last_login_at FROM partner_users WHERE tenant_id=$1 ORDER BY email", [req.params.partnerId]);
    res.json(rows);
  });
  ...
  r.get("/:partnerId/audit", async (req, res) => {
    const { rows } = await partnerAdminQuery("SELECT id, action, actor_user_id, reason, created_at FROM partner_audit_events WHERE tenant_id=$1 ORDER BY id DESC LIMIT 200", [req.params.partnerId]);
    res.json(rows);
  });

  r.get("/:partnerId/security", async (req, res) => {
    const { rows } = await partnerAdminQuery("SELECT id, severity, kind, created_at FROM partner_security_events WHERE tenant_id=$1 ORDER BY id DESC LIMIT 200", [req.params.partnerId]);
    res.json(rows);
  });
  ...
  r.post("/:partnerId/suspend", async (req, res) => {
    ...
    await partnerAdminQuery("UPDATE partner_sessions SET revoked_at=now() WHERE tenant_id=$1 AND revoked_at IS NULL", [req.params.partnerId]);
    await partnerAdminQuery("INSERT INTO partner_security_events (tenant_id, severity, kind) VALUES ($1,'high','partner_suspended')", [req.params.partnerId]);
  ...
  r.post("/:partnerId/locations/:locationId/suspend", async (req, res) => {
    ...
    const email = (req.session as { adminEmail?: string })?.adminEmail ?? "admin";
    await partnerAdminQuery("UPDATE partner_locations SET status='SUSPENDED' WHERE id=$1 AND tenant_id=$2", [req.params.locationId, req.params.partnerId]);
    // H3: revoke live sessions bound to the suspended location so it stops immediately.
    await partnerAdminQuery("UPDATE partner_sessions SET revoked_at=now() WHERE location_id=$1 AND revoked_at IS NULL", [req.params.locationId]);
    await adminAudit(req.params.partnerId, "partner_location_suspended", reason, email, "partner_location", req.params.locationId);
    res.json({ ok: true });
  });
  ...
  r.post("/:partnerId/users/:userId/suspend", async (req, res) => {
    ...
    const email = (req.session as { adminEmail?: string })?.adminEmail ?? "admin";
    await partnerAdminQuery("UPDATE partner_users SET status='SUSPENDED', credential_version=credential_version+1 WHERE id=$1 AND tenant_id=$2", [req.params.userId, req.params.partnerId]);
    ...
    await adminAudit(req.params.partnerId, "partner_user_suspended", reason, email, "partner_user", req.params.userId);
    res.json({ ok: true });
  });
  ...
  r.post("/:partnerId/emergency-stop", async (req, res) => {
    const email = (req.session as { adminEmail?: string })?.adminEmail ?? "admin";
    await partnerAdminQuery("INSERT INTO partner_emergency_controls (tenant_id, scope, frozen, set_by, reason) VALUES ($1,'partner',true,$2,$3)", [req.params.partnerId, email, String(req.body?.reason ?? "")]);
    await partnerAdminQuery("UPDATE partner_sessions SET revoked_at=now() WHERE tenant_id=$1 AND revoked_at IS NULL", [req.params.partnerId]);
    await partnerAdminQuery("INSERT INTO partner_security_events (tenant_id, severity, kind) VALUES ($1,'critical','partner_emergency_stop')", [req.params.partnerId]);
