 ...
 *
 * RECONCILED 2026-08-11 (mainline ↔ signed-station branch). Two independent hardening passes landed
 * on this file in parallel and BOTH are preserved here, because they defend different things:
 *
 *   • The PENDING-ENROLMENT LIFECYCLE (migration 0044_partner_mfa_pending_lifecycle, already applied
 *     in production): a pending secret is bound to the enrolment session that created it, expires,
 *     is confirmed by explicit id, and records consumption. This is what stops a pending secret
 *     issued to one session being confirmed by another, or living forever.
 *   • The SECOND-FACTOR GATE on replacement/regeneration (F3): swapping an authenticator, or minting
 *     a fresh set of recovery codes, now demands exactly what `mfaDisable` demands — password PLUS a
 *     live replay-protected TOTP or a single-use recovery code — because both are account-takeover
 *     primitives of the same class.
 *
 * Neither was dropped in favour of the other. BOOTSTRAP (no ACTIVE method) stays password-only on
 * every path, deliberately: there is no factor to present, so gating it would strand the user.
 */
 ...
 * proved the current factor. An abandoned PENDING row is therefore useless to anyone else — and
 * since 0044 it is additionally bound to the issuing session and expires on its own.
 */
export async function mfaEnrolStart(
  ...
  return createPendingEnrolment(ctx, password, { allowActiveReplacement: true }, secondFactor);
}
...
async function createPendingEnrolment(
  ...
  return withTenant({ tenantId: ctx.tenantId }, async (c): Promise<EnrolResult> => {
    ...
    if (await hasActiveMethod(c, ctx.userId)) {
      // Restart (allowActiveReplacement=false) is a resume-the-pending-flow affordance, never a
      // replacement primitive — it refuses outright rather than asking for a second factor.
 ...
 *
 * The pending row is looked up BY ID and must belong to this user AND to the session that started the
 * enrolment (0044's `enrolment_session_id`), and must not have expired. That binding is what makes an
 * abandoned or intercepted pending secret useless to any other session.
 *
 * F3: when this ACTIVATION REPLACES an existing factor it is a credential change, and is treated as
 * one — `credential_version` is bumped and every OTHER live session is revoked, so a second, stolen
 * session cannot outlive the authenticator swap it did not perform. The session that completed the
 * replacement is re-pinned to the new version rather than killed: it has just proved password + the
 * OLD factor (at enrol-start) + the NEW factor (here), which is strictly more than any other session
 * on the account can show. The security event is raised from 'info' to 'medium', matching
 * `partner_mfa_disabled` — the two operations carry comparable risk.
 *
...
export async function mfaDisable(
  ...
  return withTenant({ tenantId: ctx.tenantId }, async (c): Promise<DisableResult> => {
    if (!(await verifyPassword(c, ctx.userId, password))) return { ok: false, reason: "unauthorised" };
    // require an existing MFA second factor — same helper as replacement/regeneration.
    if (!(await verifySecondFactor(c, ctx.userId, secondFactor)))
      return { ok: false, reason: "second_factor_required" };
