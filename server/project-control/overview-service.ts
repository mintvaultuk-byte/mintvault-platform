...
/**
 * The DTO now lives in `shared/project-control-overview.ts` so the browser can import it — see
 * that file for why. Re-exported here so every existing server import keeps working unchanged.
 */
import type {
  EvidenceMeta,
  RepositorySection,
...
export async function buildComposedOverview(db: EvidenceDb, now: Date = new Date()): Promise<OverviewDto> {
  ...
  const repository: RepositorySection = {
    ...
    // Falls back to null rather than to the all-state total: an older snapshot predating the
  ...
  return {
    ...
    executive: {
      ...
      productionAligned: deploymentAlignment(repository.mainSha, productionApp?.commit ?? null),
      stagingMatchesProduction: deploymentAlignment(stagingApp?.commit ?? null, productionApp?.commit ?? null),
    },
    integrity: {
      staleSources,
      unknownSources,
