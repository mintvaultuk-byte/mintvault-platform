...
export default function GradeDisplay({ overall, sub, hasCrease, hasTear, manualOverride, onOverride, onSubgradeChange, gradeLabel, isBlack, strengthScore }: Props) {
  ...
  return (
    <div className="space-y-3">
      ...
      {strengthScore != null && (
        <div className="bg-[#F7F7F5] border border-[#E8E4DC] rounded-lg p-3 text-center">
          <div className="flex items-center justify-center gap-1 mb-1">
            <p className="text-[#888888] text-[10px] font-bold uppercase tracking-widest">Grade Strength</p>
            <div className="relative group">
              <Info size={10} className="text-[#888888] cursor-help" />
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 w-48 bg-[#F0EEE8] border border-[#D4D0C8] rounded p-2 text-[9px] text-[#666666] leading-relaxed hidden group-hover:block z-20">
          ...
          <p className="leading-none">
            ...
            <span className="text-sm text-[#888888] font-bold">/100</span>
          </p>
          <p className="text-[#888888] text-[9px] mt-1">Position within tier — higher = stronger</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-4 gap-1.5">
        {([
          ...
          <div key={label} className="bg-[#F7F7F5] border border-[#E8E4DC] rounded p-2 text-center">
            <p className="text-[#888888] text-[10px] font-semibold uppercase tracking-wider">{label}</p>
            <div className="flex items-center justify-center gap-1 mt-0.5">
              {onSubgradeChange && (
                <button type="button" onClick={() => onSubgradeChange(key, Math.max(1, val - 1))}
                  className="text-[#888888] hover:text-[#D4AF37] text-xs leading-none">▼</button>
              )}
      ...
      {showCalc && (
        <div className="bg-white border border-[#E8E4DC] rounded-lg p-3 text-[10px] text-[#666666] font-mono space-y-1">
          ...
          <p className="text-[#666666]">Final: {overall}</p>
        </div>
      )}
