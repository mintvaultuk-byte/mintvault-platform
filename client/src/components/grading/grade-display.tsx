...
export default function GradeDisplay({
  ...
  return (
    <div className="space-y-2">
      <div
        className={`rounded-lg px-3 py-2.5 bg-gradient-to-br ${overall > 0 ? overallBg(overall) : "from-[#333333] to-[#222222]"} text-center`}
      >
        <p className="text-[#1A1400]/70 text-[9px] font-bold uppercase tracking-widest">Overall Grade</p>
        <p className="text-3xl font-black text-[#1A1400] leading-none">{overall > 0 ? overall : "—"}</p>
        <p className="text-[#1A1400] text-[10px] font-bold uppercase tracking-widest mt-0.5">
          {overall > 0 ? gradeLabel : "Awaiting server calculation"}
        </p>
      </div>

      {strengthScore != null && (
        <div className="bg-[var(--admin-panel2)] border border-[var(--admin-line)] rounded-lg p-2 text-center">
          ...
          <p className="leading-none">
            <span className="text-2xl font-black" style={{ color: strengthColor(strengthScore) }}>
              {strengthScore}
            </span>
      ...
      {isBlack && (
        <div className="flex items-center justify-center gap-1.5 border border-[var(--admin-gold)]/50 rounded-lg px-2 py-1.5 bg-[var(--admin-gold)]/10 animate-pulse">
      ...
      <div className="grid grid-cols-4 gap-1">
        {values.map(({ label, value }) => {
          ...
          return (
            <div
              key={label}
              className="bg-[var(--admin-panel2)] border border-[var(--admin-line)] rounded px-1.5 py-1 text-center"
            >
              <p className="text-[var(--admin-ink-dim)] text-[10px] font-semibold uppercase tracking-wider">
                {label}
                {lowConfidence ? " · review" : ""}
              </p>
              <p className="text-sm font-black" style={{ color: value > 0 ? subgradeColor(value) : "#888888" }}>
                {value > 0 ? value : "—"}
              </p>
      ...
      <p className="text-[var(--admin-ink-faint)] text-[8px] italic">
