...
function ConfidenceBadge({ level }: { level: Confidence }) {
  const cls =
    level === "high"
      ? "bg-[color-mix(in_srgb,var(--admin-green)_12%,transparent)] text-[var(--admin-green)] border-[color-mix(in_srgb,var(--admin-green)_40%,transparent)]"
      : level === "medium"
  ...
  return <span className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded border ${cls}`}>{level}</span>;
}
    ...
    <div className="space-y-1">
      <button
        ...
        className={`w-full flex items-center justify-between gap-2 px-3 py-2.5 rounded-lg border text-xs font-bold uppercase transition-all ${
          status === "done"
            ? "border-[color-mix(in_srgb,var(--admin-green)_40%,transparent)] bg-[color-mix(in_srgb,var(--admin-green)_12%,transparent)] text-[var(--admin-green)]"
            : status === "error"
              ? "border-[color-mix(in_srgb,var(--admin-red)_50%,transparent)] bg-[color-mix(in_srgb,var(--admin-red)_12%,transparent)] text-[var(--admin-red)]"
              : status === "loading"
                ? "border-[var(--admin-gold)]/40 bg-[var(--admin-gold)]/5 text-[var(--admin-gold)]"
                : "border-[var(--admin-line)] text-[var(--admin-ink-dim)] hover:border-[var(--admin-gold)]/40 hover:text-[var(--admin-gold)]"
        }`}
      >
        ...
        <span className="text-[9px] text-[var(--admin-ink-dim)] font-normal normal-case">{cost}</span>
      </button>
      {status === "error" && err && <p className="text-[var(--admin-red)] text-[10px] px-1">{err}</p>}
    </div>
  );
}
...
export default function AiPanel({
  ...
  const [panelTcgResults, setPanelTcgResults] = useState<
    {
      id: string;
      name: string;
      setName: string;
  ...
  function handlePanelTcgSearch(query: string) {
    ...
    if (query.trim().length < 3) {
      setPanelTcgResults([]);
      setPanelTcgLoading(false);
      return;
    }
    const gameSlug = (cardGame || "pokemon")
      .toLowerCase()
      .replace(/[éè]/g, "e")
      .replace(/[^a-z0-9]/g, "");
    const cacheKey = `${gameSlug}:${query.trim().toLowerCase()}`;
    if (panelTcgCache.has(cacheKey)) {
      setPanelTcgResults(panelTcgCache.get(cacheKey)!);
      return;
    }
  ...
  function emptyAnalysis(): AiAnalysisResult {
    ...
    return {
      ...
      corners: {
        ...
        front_top_left: dc,
        front_top_right: dc,
        front_bottom_left: dc,
        front_bottom_right: dc,
        back_top_left: dc,
        back_top_right: dc,
        back_bottom_left: dc,
        back_bottom_right: dc,
        notes: "",
      },
      edges: {
        subgrade: 0,
  ...
  async function runCentering() {
    ...
    try {
      ...
      setCenteringStatus("error");
      setCenteringError(e.message);
    }
  }
  ...
  async function runDefects() {
    ...
    try {
      ...
      setDefectsStatus("error");
      setDefectsError(e.message);
    }
  }
  ...
  async function callGradeEndpoint() {
    ...
    const synthesised: AiAnalysisResult = {
      ...
      corners: {
        subgrade: coGrade,
        front_top_left: cornerVal,
        front_top_right: cornerVal,
        front_bottom_left: cornerVal,
        front_bottom_right: cornerVal,
        back_top_left: cornerVal,
        back_top_right: cornerVal,
        back_bottom_left: cornerVal,
        back_bottom_right: cornerVal,
        notes: "",
      },
      edges: {
        subgrade: edGrade,
  ...
  async function runAnalysis() {
    ...
    const steps: Step[] = ["analyzing_centering", "analyzing_corners", "analyzing_surface", "generating_report"];
    let stepIdx = 0;
    ...
    try {
      ...
      if (data.detailsWritten === false) {
        toast({
          title: "AI couldn't confidently identify this card",
  ...
  // AI grading triggers are hidden, so the panel has nothing to show until the
  // server pre-grade / identification arrives — avoid rendering an empty box.
  ...
  return (
    <div className="bg-[var(--admin-panel)] border border-[var(--admin-gold)]/20 rounded-xl p-4 space-y-4">
      {/* AI grading trigger buttons removed — paid AI grading is disabled in the UI
          for all users (graders + admin). The externalAnalysis consumer (useEffect
          above) still feeds the server-computed pre-grade into the manual panel
          (pins, sub-grades, identity), so downstream AI data paths stay intact —
          only the click-to-pay buttons are gone. */}

      {/* Confidence indicator */}
      {step === "complete" && idConfidence && (
        <div className="flex items-center gap-2 text-xs">
          <span
            className={`w-2 h-2 rounded-full ${idConfidence === "high" ? "bg-[var(--admin-green)]" : idConfidence === "medium" ? "bg-[var(--admin-amber)]" : "bg-[var(--admin-red)]"}`}
          />
          <span
            className={
              idConfidence === "high"
          ...
          >
            ID confidence: {idConfidence}
            {idVerified ? " (TCG API verified)" : ""}
          </span>
          {idConfidence === "low" && (
            <span className="text-[var(--admin-red)]/70">— manual verification recommended</span>
          )}
        </div>
      )}
      ...
      {/* Loading progress */}
      {isLoading && (
        <div className="flex items-center gap-2 text-[var(--admin-ink-dim)] text-xs">
      ...
      {/* Error */}
      {step === "error" && (
        <div className="flex items-center gap-2 bg-[color-mix(in_srgb,var(--admin-red)_12%,transparent)] border border-[color-mix(in_srgb,var(--admin-red)_40%,transparent)] rounded-lg px-3 py-2.5">
          <AlertTriangle size={14} className="text-[var(--admin-red)] flex-shrink-0" />
          <p className="text-[var(--admin-red)] text-xs">{error}</p>
        </div>
      )}
        ...
        <div className="space-y-4">
          {identification && (
            <div className="bg-[var(--admin-panel2)] border border-[var(--admin-line)] rounded-lg p-3 space-y-2">
              <div className="flex items-center justify-between">
                <p className="text-[var(--admin-gold)]/70 text-[10px] font-bold uppercase tracking-widest">
                  Card Identified
                </p>
                <div className="flex items-center gap-2">
                  {(identification as any).manuallyVerified ||
                  (identification as any).dbSource === "manual-tcg-search" ? (
                    <span className="flex items-center gap-1 text-[var(--admin-green)] text-[10px]">
                    ...
                    <span className="flex items-center gap-1 text-[var(--admin-amber)] text-[10px]">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      setPanelTcgOpen(true);
                      setPanelTcgQuery("");
                      setPanelTcgResults([]);
                    }}
                    className="flex items-center gap-1 text-[var(--admin-gold)] text-[10px] font-bold hover:text-[var(--admin-gold-deep)] transition-colors"
                  >
                    <Database size={9} />
                    Change
                  </button>
              ...
              <div className="flex gap-3">
                {identification.referenceImageUrl && (
                  <img
                    ...
                    alt="Reference"
                    className="w-16 h-22 object-contain rounded border border-[var(--admin-line)] flex-shrink-0"
                  />
                )}
                <div className="space-y-0.5 min-w-0">
                  <p className="text-[var(--admin-ink)] text-sm font-bold truncate">{identification.officialName}</p>
                  <p className="text-[var(--admin-ink-dim)] text-xs">
                    {identification.officialSet}
                    {identification.officialNumber ? ` · ${identification.officialNumber}` : ""}
                  </p>
                  <p className="text-[var(--admin-ink-dim)] text-[10px]">
                    {identification.detected_game?.toUpperCase()} · {identification.detected_language} ·{" "}
                    {identification.detected_rarity}
                  </p>
                  {identification.is_holo && (
                    <span className="text-[9px] text-[var(--admin-gold)] bg-[var(--admin-gold)]/10 px-1.5 py-0.5 rounded">
                      HOLO
                    </span>
                  )}
                </div>
          ...
          {(referenceImageUrl || identification?.referenceImageUrl) && (
            <a
              ...
              className="flex items-center gap-1.5 text-[var(--admin-gold)]/60 text-xs hover:text-[var(--admin-gold)] transition-colors"
            >
            ...
            <>
              <div className="grid grid-cols-5 gap-2">
                {[
                  ...
                  <div key={label} className="bg-[var(--admin-panel2)] rounded-lg p-2 text-center">
                    <p className="text-[var(--admin-ink-dim)] text-[8px] uppercase tracking-widest">{label}</p>
                    <p className="text-[var(--admin-gold)] text-lg font-black">{value}</p>
              ...
              {(!result.is_authentic || result.is_altered) && (
                <div className="bg-[color-mix(in_srgb,var(--admin-amber)_12%,transparent)] border border-[color-mix(in_srgb,var(--admin-amber)_40%,transparent)] rounded-lg px-3 py-2.5">
                  <p className="text-[var(--admin-amber)] text-xs font-bold">
                    {!result.is_authentic ? "⚠ Potential counterfeit detected" : "⚠ Card may be altered"}
                  </p>
                  {result.authentication_notes && (
                    <p className="text-[var(--admin-amber)]/70 text-xs mt-1">{result.authentication_notes}</p>
                  )}
                </div>
              )}
              ...
              {result.confidence_notes && (
                <div>
                  <button
                    ...
                    className="flex items-center gap-1 text-[var(--admin-ink-dim)] hover:text-[var(--admin-ink-dim)] text-[10px] transition-colors"
                  >
                    {showConfidenceNotes ? <ChevronUp size={11} /> : <ChevronDown size={11} />}
                    Confidence notes
                  </button>
                  {showConfidenceNotes && (
                    <p className="text-[var(--admin-ink-dim)] text-xs mt-1 leading-relaxed">
                      {result.confidence_notes}
                    </p>
                  )}
                </div>
              ...
              {/* Centering detail */}
              <div className="bg-[var(--admin-panel2)] border border-[var(--admin-line)] rounded-lg p-3 space-y-1">
                <p className="text-[var(--admin-gold)]/70 text-[10px] font-bold uppercase tracking-widest">Centering</p>
                <div className="grid grid-cols-2 gap-x-4 text-xs">
                  <span className="text-[var(--admin-ink-dim)]">Front L/R</span>
                  <span className="text-[var(--admin-ink)]">{result.centering.front_left_right}</span>
                  <span className="text-[var(--admin-ink-dim)]">Front T/B</span>
                  <span className="text-[var(--admin-ink)]">{result.centering.front_top_bottom}</span>
                  <span className="text-[var(--admin-ink-dim)]">Back L/R</span>
                  <span className="text-[var(--admin-ink)]">{result.centering.back_left_right}</span>
                  <span className="text-[var(--admin-ink-dim)]">Back T/B</span>
                  <span className="text-[var(--admin-ink)]">{result.centering.back_top_bottom}</span>
                </div>
                {result.centering.notes && (
                  <p className="text-[var(--admin-ink-dim)] text-[10px] mt-1">{result.centering.notes}</p>
                )}
              </div>
                      ...
                      <div key={d.id} className="flex items-start gap-2">
                        <span
                          className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded flex-shrink-0 mt-0.5 ${
                            d.severity === "major"
                              ? "bg-[color-mix(in_srgb,var(--admin-red)_12%,transparent)] text-[var(--admin-red)] border border-[color-mix(in_srgb,var(--admin-red)_40%,transparent)]"
                              : d.severity === "moderate"
                                ? "bg-[color-mix(in_srgb,var(--admin-amber)_12%,transparent)] text-[var(--admin-amber)] border border-[color-mix(in_srgb,var(--admin-amber)_40%,transparent)]"
                                : "bg-[var(--admin-panel3)] text-[var(--admin-ink-dim)] border border-[var(--admin-line)]"
                          }`}
                        >
                          {d.severity}
                        </span>
                        <div className="min-w-0">
                          <p className="text-[var(--admin-ink)] text-xs font-medium capitalize">
                            {d.type.replace(/_/g, " ")} — {d.location}
                          </p>
                          <p className="text-[var(--admin-ink-dim)] text-[10px] leading-snug">{d.description}</p>
                          {d.detected_in !== "original" && (
                            <p className="text-[var(--admin-ink-faint)] text-[9px]">detected in: {d.detected_in}</p>
                          )}
                        </div>
                      </div>
                    ))}
              ...
              {result.grade_explanation && (
                <div className="bg-[var(--admin-panel2)] border border-[var(--admin-line)] rounded-lg p-3">
                  <p className="text-[var(--admin-gold)]/70 text-[10px] font-bold uppercase tracking-widest mb-1.5">
                    AI Grade Explanation
                  </p>
                  <p className="text-[var(--admin-ink-dim)] text-xs leading-relaxed">{result.grade_explanation}</p>
                </div>
              )}
              ...
              {result.recommendations?.length > 0 && (
                <div className="space-y-1">
                  ...
                  {result.recommendations.map((r, i) => (
                    <p key={i} className="text-[var(--admin-ink-dim)] text-xs">
                      · {r}
                    </p>
                  ))}
                </div>
              ...
              {result.authentication_notes && result.is_authentic && !result.is_altered && (
                <div>
                  <button
                    ...
                    className="flex items-center gap-1 text-[var(--admin-ink-faint)] hover:text-[var(--admin-ink-dim)] text-[10px] transition-colors"
                  >
          ...
          <div className="p-4 pb-2 border-b border-[var(--admin-line)]">
            ...
            <div className="flex items-center gap-2 border border-[var(--admin-gold)]/30 rounded-lg px-3 py-2 focus-within:border-[var(--admin-gold)] transition-colors">
              ...
              <input
                ...
                autoFocus
              />
              {panelTcgLoading && <Loader2 size={14} className="animate-spin text-[var(--admin-gold)]" />}
            </div>
          </div>
