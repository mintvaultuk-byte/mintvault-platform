...
export default function CrossGradeDisplay({ mvGrade, subgrades, strengthScore, compact }: Props) {
  ...
  if (compact) {
    return (
      <div className="flex items-center gap-3 text-[10px]">
        <span className="text-[#FFCB05] font-bold">MV {mvGrade}</span>
  ...
  return (
    <div className="bg-[#F7F7F5] border border-[#E8E4DC] rounded-lg p-3">
      <p className="text-[9px] font-bold text-[#FFCB05] uppercase tracking-wider mb-2">Cross-Grade Estimate</p>
      <div className="grid grid-cols-4 gap-2 text-center">
        <div>
          <p className="text-[8px] text-[#888888] uppercase">MintVault</p>
          <p className="text-lg font-black text-[#FFCB05]">{mvGrade}</p>
        </div>
        <div>
