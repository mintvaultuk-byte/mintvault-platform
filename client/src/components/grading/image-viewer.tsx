      ...
      >
        {currentUrl ? (
          <div className="relative w-full h-full" style={{ transform: transformStyle, transition: transitionStyle }}>
            ...
            {/* Defect ring markers — clickable for deletion when not in markMode */}
            {showDefects && (() => {
              let humanIdx = 0;
              return sideDefects.map(d => {
                ...
                const showPopup = deleteConfirmId === d.id;
                return (
                  <div key={d.id}
                    className={`absolute ${markMode ? "pointer-events-none" : "cursor-pointer"} ${isHL ? "defect-ring-pulse" : ""}`}
                    style={{ left: `${d.x_percent}%`, top: `${d.y_percent}%`, transform: "translate(-50%, -50%)", width: 32, height: 32, zIndex: showPopup ? 30 : 10 }}
                    onClick={!markMode ? (e) => { e.stopPropagation(); setDeleteConfirmId(showPopup ? null : d.id); } : undefined}
                  >
                    ...
                    {showPopup && onDefectDeleted && (
                      <div className="absolute top-8 left-1/2 -translate-x-1/2 bg-[#111111] border border-[#333333] rounded-lg p-2 shadow-xl whitespace-nowrap z-30"
                        ...
                        <p className="text-[#CCCCCC] text-[10px] mb-1.5">{d.type || "Defect"} ({d.severity || "?"})</p>
                        <div className="flex gap-1">
                          <button type="button"
                            onClick={() => { onDefectDeleted(d.id); setDeleteConfirmId(null); }}
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {certId && hasAny(urls, side) && urls[`${side}_original` as keyof ImageUrls] && (
      ...
      {/* Manual Crop modal */}
      {manualCropSide && certId && (() => {
        const rawUrl = urls[`${manualCropSide}_original` as keyof ImageUrls] as string | null;
