...
export default function ManualCrop({ side, certId, rawImageUrl, onDone, onCancel }: Props) {
  ...
  async function handleAutoDetect() {
    ...
    try {
      ...
      if (j.ok && j.bounds) {
        setBox({
          left: j.bounds.left_pct,
          top: j.bounds.top_pct,
          right: j.bounds.left_pct + j.bounds.width_pct,
          bottom: j.bounds.top_pct + j.bounds.height_pct,
        });
    ...
    <div className="fixed inset-0 z-[100] bg-black/95 flex flex-col select-none">
      ...
      <div className="flex-shrink-0 px-4 py-3 flex items-center justify-between border-b border-[#333333]">
        <div>
          ...
          <p className="text-[#888888] text-[10px]">Drag corners to fit the actual card edges. Use rotation to straighten tilted cards. Esc to cancel.</p>
        </div>
        <button type="button" onClick={onCancel} className="text-[#888888] hover:text-white p-1"><X size={20} /></button>
      </div>

      {/* Image area — full image visible, no fixed aspect ratio */}
