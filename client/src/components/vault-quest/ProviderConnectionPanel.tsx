...
import { estimatedCreditsForGenerateWith, type VqAiProviderId, type VqProviderDescriptor } from "@shared/vq-ai-provider";
...
export function ProviderConnectionPanel({ onStatusChange }: { onStatusChange?: (provider: ProviderConnection | null, loaded: boolean) => void }) {
  ...
  const visibleProvider = registry?.availableProviders.find((p) => p.id === selectedProvider && p.available) ?? registry?.availableProviders.find((p) => p.id === "higgsfield") ?? null;
  ...
  return (
    <div className={`rounded-xl border p-3 ${tone.box}`}>
      ...
      <div className="grid grid-cols-1 gap-2 text-xs sm:grid-cols-2 lg:grid-cols-4">
        <div><div className="text-[10px] uppercase tracking-wide text-slate-500">Provider</div><div className="font-semibold text-slate-200">{visibleProvider?.recommended ? "★ " : ""}{visibleProvider?.label ?? "Higgsfield"}</div></div>
        ...
        <div><div className="text-[10px] uppercase tracking-wide text-slate-500">Model</div><div className="font-semibold text-slate-200">Auto</div></div>
        <div><div className="text-[10px] uppercase tracking-wide text-slate-500">Credits</div><div className="font-semibold text-slate-200">Unavailable</div></div>
        <div><div className="text-[10px] uppercase tracking-wide text-slate-500">Estimated next generation</div><div className="font-semibold text-slate-200">≈{estimatedNextCredits} credits</div></div>
