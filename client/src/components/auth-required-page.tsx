...
export default function AuthRequiredPage({ currentPath }: { currentPath?: string }) {
  ...
  return (
    <div className="min-h-[60vh] bg-[#FAFAF8] flex items-center justify-center px-4 py-16">
      <div className="w-full max-w-sm bg-white rounded-2xl border border-[#FFCB05]/30 shadow-lg p-8 text-center">
        <div className="w-14 h-14 rounded-full bg-[#FFCB05]/10 border border-[#FFCB05]/30 flex items-center justify-center mx-auto mb-5">
          <Lock size={22} className="text-[#FFCB05]" />
        </div>
        <h2
        ...
        <div className="flex flex-col gap-3">
          ...
          <Link href={`/signup${next}`}>
            <button className="w-full py-3 rounded-xl font-bold text-sm text-[#FFCB05] border border-[#FFCB05]/40 hover:bg-[#FFCB05]/5 transition-all">
              Create Account
            </button>
          </Link>
        </div>
