...
export const partnerAuth = {
  ...
  mfa: (input: { code?: string; recoveryCode?: string }) => req<{ ok: boolean }>("POST", "/api/partner/auth/mfa", input),
  logout: () => req<{ ok: boolean }>("POST", "/api/partner/auth/logout"),
  session: () => req<PartnerSessionInfo>("GET", "/api/partner/session"),
  switchLocation: (locationId: string) => req<{ ok: boolean; locationId: string }>("POST", "/api/partner/session/location", { locationId }),
...
export const partnerCustomers = {
  list: (search?: string) => req<PartnerCustomer[]>("GET", `/api/partner/customers${search ? `?search=${encodeURIComponent(search)}` : ""}`),
...
export const partnerSubmissions = {
  ...
  create: (input: { locationId: string; customerId?: string | null; internalReference?: string | null; serviceTierCode?: string | null; intakeNotes?: string | null }) =>
  ...
  edit: (
    id: string,
    input: { version: number; customerId?: string | null; internalReference?: string | null; serviceTierCode?: string | null; intakeNotes?: string | null },
  ) => req<SubmissionSummary>("PATCH", `/api/partner/submissions/${id}`, input),
  cancel: (id: string, reason: string) => req<SubmissionSummary>("DELETE", `/api/partner/submissions/${id}`, { reason }),
  submit: (id: string, idempotencyKey: string) => req<SubmissionDetail>("POST", `/api/partner/submissions/${id}/submit`, { idempotencyKey }),
};
...
export const partnerCards = {
  ...
  remove: (submissionId: string, cardId: string, reason?: string) =>
    req<{ ok: boolean }>("DELETE", `/api/partner/submissions/${submissionId}/cards/${cardId}`, reason ? { reason } : undefined),
};
