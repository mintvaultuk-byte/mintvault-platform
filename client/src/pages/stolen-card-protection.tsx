  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      >
        ...
        <div className="relative max-w-3xl mx-auto px-4 py-20 text-center">
          ...
          <p className="text-[#B8A060] text-lg max-w-xl mx-auto">
            If your graded card is stolen, MintVault can flag its certificate so any buyer who scans the QR code or NFC
            chip sees an instant warning.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-16 space-y-16">
        {/* How it works */}
        <section>
          <h2 className="text-2xl font-black text-[#1A1A1A] mb-2" style={{ letterSpacing: "-0.02em" }}>
            How It Works
          </h2>
            ...
            {[
              {
                step: "1",
                title: "Report",
                desc: "Submit a stolen report below with your name, email, and the certificate ID on the label.",
              },
              {
                step: "2",
                title: "Verify",
                desc: "We email you a one-click verification link. Click it to confirm the report is genuine.",
              },
              {
                step: "3",
                title: "Protected",
                desc: "A bright red banner appears on the certificate's Vault page and any NFC/QR scan shows a stolen warning.",
              },
            ].map(({ step, title, desc }) => (
              <div key={step} className="border border-[#E8E4DC] rounded-xl p-5">
                <div className="w-8 h-8 rounded-full bg-[#D4AF37] text-[#1A1400] font-black text-sm flex items-center justify-center mb-3">
                  {step}
                </div>
        ...
        <section id="report">
          ...
          <p className="text-[#666666] text-sm mb-6">
            The certificate ID is printed on your MintVault label (e.g. MV1, MV-0000000001).
          </p>
          <ReportForm />
        </section>
        ...
        <section id="check">
          ...
          <p className="text-[#666666] text-sm mb-6">
            Enter a certificate ID to check whether it has been reported stolen.
          </p>
        ...
        <section id="stolen-cards">
          <h2 className="text-2xl font-black text-[#1A1A1A] mb-6" style={{ letterSpacing: "-0.02em" }}>
        ...
        <section
          ...
          style={{ background: "linear-gradient(135deg,#0A0A0A,#1A1200)", border: "1px solid #D4AF37" }}
        >
          ...
          <p className="text-[#B8A060] text-sm max-w-md mx-auto mb-6">
            Claiming ownership of your graded card links it to your identity — making it much easier to prove provenance
            if your card is ever stolen or disputed.
          </p>
          <a href="/claim">
            <GradientButton className="gradient-btn-filled">Claim Your Card</GradientButton>
          </a>
        </section>
      </div>
    </div>
    ...
    return (
      <div className="border border-emerald-200 bg-emerald-50 rounded-xl p-6 text-center max-w-md">
        ...
        <p className="text-sm text-[#666666]">
          Check your inbox — we've sent a verification link to <strong>{email}</strong>. Click it to confirm and flag
          the certificate.
        </p>
      </div>
    );
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-md space-y-4">
      <div>
        ...
        <CertIdInput value={certId} onChange={setCertId} placeholder="1" className="w-full rounded-lg" />
      </div>
      <div>
