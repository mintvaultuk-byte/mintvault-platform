...
function divCellClass(d: number | null): string {
  ...
  // than "we missed by a full grade point". Dark-theme tints use the admin
  // status tokens (green/amber/blue/red) at low alpha.
  if (d == null) return "bg-[rgba(243,238,227,0.05)] text-[var(--admin-ink-faint)]";
  const a = Math.abs(d);
  if (a === 0) return "bg-[rgba(134,189,144,0.16)] text-[var(--admin-green)]";
  if (a < 0.5) return "bg-[rgba(134,189,144,0.08)] text-[var(--admin-green)]";
  if (a < 1.0)
    return d > 0
      ? "bg-[rgba(227,183,95,0.14)] text-[var(--admin-amber)]"
      : "bg-[rgba(138,166,196,0.14)] text-[var(--admin-blue)]";
  return "bg-[rgba(205,128,115,0.16)] text-[var(--admin-red)] font-bold";
}

function biasRowClass(mean: number | null): string {
  if (mean == null) return "text-[var(--admin-ink-faint)]";
  const a = Math.abs(mean);
  if (a < 0.1) return "text-[var(--admin-green)]";
  if (mean > 0) return "text-[var(--admin-amber)]";
  return "text-[var(--admin-blue)]";
}
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3">
          <TrendingUp size={22} className="text-[var(--admin-gold-hi)]" />
          <div>
            <h1 className="text-xl font-bold text-[var(--admin-ink)]">AI Divergence</h1>
      ...
      {!isLoading && data && (
        <>
          {/* ── Sample size warning ───────────────────────────────────────── */}
          {data.sample_size < 5 && (
            <div className="flex items-start gap-2 bg-[rgba(227,183,95,0.08)] border border-[rgba(227,183,95,0.3)] rounded-xl px-4 py-3">
              <AlertTriangle size={16} className="text-[var(--admin-amber)] mt-0.5 flex-shrink-0" />
              <div>
                <p className="text-[var(--admin-amber)] text-sm font-bold">Not enough paired data yet</p>
          ...
          <p className="text-[var(--admin-ink-faint)] text-sm">
            Sample: <strong>{data.sample_size}</strong> cert{data.sample_size === 1 ? "" : "s"} with paired prediction +
            human grade
          ...
          {data.sample_size > 0 && summary && (
            <>
              {/* ── Bias by zone ─────────────────────────────────────────── */}
              <Panel>
                <h2 className="text-[var(--admin-ink)] font-bold mb-4">Bias by zone</h2>
                <table className="w-full text-sm">
                  <thead>
                    <tr
                      className="text-[10px] uppercase tracking-widest text-[var(--admin-ink-faint)] border-b border-[var(--admin-line)]"
                  ...
                  <tbody>
                    {ZONES.map((z) => {
                      ...
                      return (
                        <tr key={z} className="border-b border-[var(--admin-line)] last:border-0">
                          <td className="px-2 py-2 font-medium text-[var(--admin-ink)]">{ZONE_LABEL[z]}</td>
                          <td className="px-2 py-2 text-right tabular-nums text-[var(--admin-ink)]">{s.n}</td>
                          <td
                            className={`px-2 py-2 text-right tabular-nums font-bold ${biasRowClass(s.mean_divergence)}`}
                          >
                            {fmtSigned(s.mean_divergence)}
                          </td>
                          <td className="px-2 py-2 text-right tabular-nums text-[var(--admin-ink-dim)]">
                            {fmtSigned(s.median_divergence)}
                          </td>
                          <td className="px-2 py-2 text-right tabular-nums text-[var(--admin-ink-dim)]">
                            {s.stddev != null ? s.stddev.toFixed(2) : "—"}
                          </td>
                          <td className="px-2 py-2 text-right tabular-nums text-[var(--admin-amber)]">
                            {s.ai_too_generous_pct != null ? `${s.ai_too_generous_pct}%` : "—"}
                          </td>
                          <td className="px-2 py-2 text-right tabular-nums text-[var(--admin-blue)]">
                            {s.ai_too_harsh_pct != null ? `${s.ai_too_harsh_pct}%` : "—"}
                          </td>
                          <td className="px-2 py-2 text-right tabular-nums text-[var(--admin-green)]">
                            {s.exact_match_pct != null ? `${s.exact_match_pct}%` : "—"}
                          </td>
                        </tr>
                      );
                    })}
                ...
                <p className="text-[10px] text-[var(--admin-ink-faint)] mt-3">
                  Positive mean = AI gave a higher grade than the human (too generous). Negative = AI was harsher than
                  human.
                </p>
              </Panel>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Panel>
                  <h3 className="text-[var(--admin-ink)] font-bold mb-3">By grade band</h3>
                  <div className="space-y-2 text-sm">
                    {Object.entries(summary.by_grade_band).map(([band, s]) => (
                      <div key={band} className="grid grid-cols-[80px_60px_1fr] items-center gap-2">
                        <span className="text-[var(--admin-ink)] font-medium">Grade {band}</span>
                        <span className="text-[var(--admin-ink-faint)] text-xs">n={s.n}</span>
                        <span
                          className={`text-right tabular-nums font-bold ${biasRowClass(s.mean_overall_divergence)}`}
                        >
                ...
                <Panel>
                  <h3 className="text-[var(--admin-ink)] font-bold mb-3">By model</h3>
                  <div className="space-y-2 text-sm">
                    {Object.entries(summary.by_model).map(([m, s]) => (
                      <div key={m} className="grid grid-cols-[1fr_60px_120px] items-center gap-2">
                        <span className="text-[var(--admin-ink)] font-medium truncate" title={m}>
                          {modelLabel(m)}
                        </span>
                        <span className="text-[var(--admin-ink-faint)] text-xs">n={s.n}</span>
                        <span
                    ...
                    {Object.keys(summary.by_model).length === 0 && (
                      <p className="text-[var(--admin-ink-faint)] text-xs">No predictions yet.</p>
                    )}
                  </div>
                </Panel>
              </div>
              ...
              <div className="bg-[var(--admin-panel)] border border-[var(--admin-line)] rounded-xl overflow-hidden">
                <div className="flex items-center justify-between px-6 py-3 border-b border-[var(--admin-line)]">
                  <h3 className="text-[var(--admin-ink)] font-bold">Per-cert divergence</h3>
                  <span className="text-[10px] text-[var(--admin-ink-faint)]">
                    sorted by biggest disagreement first · click row to inspect cert
                  </span>
                </div>
                <table className="w-full text-sm">
                  <thead className="bg-[var(--admin-panel2)]">
                    <tr
                    ...
                    >
                      ...
                      {ZONES.map((z) => (
                        <th key={z} className="text-center px-2 py-2 w-16" title={ZONE_LABEL[z]}>
                          {ZONE_SHORT[z]}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {certs.map((cert) => (
                      <tr
                        key={cert.cert_id}
                        className="border-b border-[var(--admin-line)] last:border-0 hover:bg-[rgba(212,175,55,0.04)] cursor-pointer"
                        onClick={() => window.open(`/cert/${cert.cert_id}`, "_blank")}
                        title={`Open ${cert.cert_id} in new tab`}
                      >
                        <td
                          className="px-3 py-2 text-xs text-[var(--admin-gold-hi)]"
                          style={{ fontFamily: "var(--admin-mono)" }}
                        >
                          <span className="inline-flex items-center gap-1">
                            {cert.cert_id} <ExternalLink size={10} className="text-[var(--admin-ink-faint)]" />
                          </span>
                        </td>
                        <td className="px-3 py-2 text-[var(--admin-ink)] truncate max-w-[200px]">
                          {cert.card_name || <span className="text-[var(--admin-ink-faint)]">—</span>}
                        </td>
                        <td className="px-3 py-2 text-[var(--admin-ink-dim)] text-xs">{modelLabel(cert.model)}</td>
                        <td className="px-3 py-2 text-[var(--admin-ink-faint)] text-xs whitespace-nowrap">
                          {fmtRelative(cert.approved_at)}
                        </td>
                        {ZONES.map((z) => {
                          ...
                          return (
                            <td key={z} className="px-1 py-1 text-center">
                              <span
                                ...
                                title={
                                  cell.ai != null && cell.human != null
                                    ? `AI ${cell.ai} vs human ${cell.human}`
                                    : "no AI prediction"
                                }
                              >
                ...
                {certs.length === 0 && (
                  <div className="p-12 text-center text-[var(--admin-ink-faint)] text-sm">
          ...
          {data.sample_size === 0 && (
            <div className="bg-[var(--admin-panel)] border border-[var(--admin-line)] rounded-xl p-12 text-center">
              <TrendingUp size={32} className="text-[var(--admin-ink-faint)] mx-auto mb-3" />
              <p className="text-[var(--admin-ink)] text-sm font-bold mb-1">No paired data in this window</p>
