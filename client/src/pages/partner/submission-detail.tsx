...
export default function PartnerSubmissionDetailPage() {
  ...
  return (
    <div className="space-y-6" data-testid="page-submission-detail">
      <Link href="/partner/submissions" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground" data-testid="link-back-to-submissions">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <Badge data-testid={`badge-detail-status-${submission.status}`}>{STATUS_LABELS[submission.status] ?? submission.status}</Badge>
      </div>
          ...
          <ul className="divide-y" data-testid="list-detail-cards">
            {cards.map((c) => (
              <li key={c.id} className="py-3">
                ...
                <p className="text-xs text-muted-foreground">{[c.game, c.card_set, c.year].filter(Boolean).join(" · ")}</p>
                {c.intake_notes && (
                  <p className="text-xs text-muted-foreground italic mt-1">Intake observation — not a MintVault grade: {c.intake_notes}</p>
                )}
              </li>
            ))}
          </ul>
