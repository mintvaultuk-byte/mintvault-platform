...
export default function PartnerSubmissionWizardPage() {
  ...
  const saveField = useCallback(
    async (patch: { customerId?: string | null; internalReference?: string | null; serviceTierCode?: string | null }) => {
  ...
  if (submitResult) {
    return <SubmitSuccessPanel submission={submitResult} onStartAnother={() => window.location.reload()} navigate={navigate} />;
  }
  ...
  return (
    <div className="space-y-6" data-testid="wizard-new-submission">
      ...
      {step === 4 && <SubmitStep missing={missing} submitting={submitting} error={submitError} onSubmit={handleSubmit} />}
          ...
          <li key={label} className="flex items-center gap-2">
            <span
              ...
              className={`flex items-center gap-1.5 px-2 py-1 rounded-full ${
                i === currentStep ? "bg-primary text-primary-foreground font-medium" : i < currentStep ? "text-primary" : "text-muted-foreground"
              }`}
            >
          ...
          <div className="space-y-3">
            ...
            {!props.creatingCustomer ? (
              ...
              <div className="space-y-3 rounded-md border p-3">
                ...
                <Button onClick={props.onCreateCustomer} disabled={!props.newCustomerName.trim()} data-testid="button-save-new-customer">
                  Save customer
                </Button>
              </div>
            )}
...
// ---------------- Step 2: Service ----------------
function ServiceStep({ tiers, selected, onSelect }: { tiers: AvailableServiceTier[]; selected: string | null; onSelect: (code: string) => void }) {
  return (
...
// ---------------- Step 3: Cards ----------------
const emptyCardForm = { cardName: "", game: PARTNER_CARD_CATEGORIES[0].value as string, cardSet: "", cardNumber: "", year: "", variant: "", language: "", declaredValuePence: "", quantity: "1", customerNotes: "", intakeNotes: "" };
        ...
        <ul className="space-y-2" data-testid="list-wizard-cards">
          {props.cards.map((c) =>
            props.editingCardId === c.id ? (
              <CardEditRow key={c.id} card={c} onSave={(input) => props.onEditCard(c.id, input)} onCancel={props.onCancelEdit} />
            ) : (
              <li key={c.id} className="flex items-center justify-between rounded-md border p-3" data-testid={`card-row-${c.id}`}>
                <div>
                  ...
                  {c.intake_notes && <p className="text-xs text-muted-foreground italic">Intake observation — not a MintVault grade: {c.intake_notes}</p>}
                </div>
                <div className="flex items-center gap-1">
                  <Button variant="ghost" size="icon" aria-label={`Edit ${c.card_name}`} onClick={() => props.onStartEdit(c.id)} data-testid={`button-edit-card-${c.id}`}>
                    <Pencil className="h-4 w-4" aria-hidden="true" />
                  </Button>
                  <Button variant="ghost" size="icon" aria-label={`Remove ${c.card_name}`} onClick={() => props.onRemoveCard(c.id)} data-testid={`button-remove-card-${c.id}`}>
        ...
        <div className="rounded-md border p-4 space-y-3" data-testid="form-add-card">
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <Input id="card-name" value={form.cardName} onChange={(e) => setForm({ ...form, cardName: e.target.value })} data-testid="input-card-name" />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Input id="card-set" value={form.cardSet} onChange={(e) => setForm({ ...form, cardSet: e.target.value })} data-testid="input-card-set" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="card-number">Card number</Label>
              <Input id="card-number" value={form.cardNumber} onChange={(e) => setForm({ ...form, cardNumber: e.target.value })} data-testid="input-card-number" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="card-year">Year</Label>
              <Input id="card-year" type="number" value={form.year} onChange={(e) => setForm({ ...form, year: e.target.value })} data-testid="input-card-year" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="card-variant">Variant</Label>
              <Input id="card-variant" value={form.variant} onChange={(e) => setForm({ ...form, variant: e.target.value })} data-testid="input-card-variant" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="card-language">Language</Label>
              <Input id="card-language" value={form.language} onChange={(e) => setForm({ ...form, language: e.target.value })} data-testid="input-card-language" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="card-quantity">Quantity</Label>
              <Input id="card-quantity" type="number" min={1} value={form.quantity} onChange={(e) => setForm({ ...form, quantity: e.target.value })} data-testid="input-card-quantity" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="card-value">Declared value (£)</Label>
              <Input id="card-value" type="number" step="0.01" value={form.declaredValuePence} onChange={(e) => setForm({ ...form, declaredValuePence: e.target.value })} data-testid="input-card-declared-value" />
            </div>
          </div>
          <div className="space-y-2">
            <Label htmlFor="card-customer-notes">Customer notes</Label>
            <Textarea id="card-customer-notes" value={form.customerNotes} onChange={(e) => setForm({ ...form, customerNotes: e.target.value })} data-testid="input-card-customer-notes" />
          </div>
          <div className="space-y-2">
...
function CardEditRow({ card, onSave, onCancel }: { card: SubmissionCard; onSave: (input: any) => Promise<void>; onCancel: () => void }) {
  ...
  return (
    <li className="flex flex-col gap-2 rounded-md border p-3" data-testid={`card-edit-row-${card.id}`}>
      <div className="flex items-center gap-2">
        <Input value={name} onChange={(e) => setName(e.target.value)} aria-label="Card name" data-testid={`input-edit-card-name-${card.id}`} />
        <Button
          variant="ghost"
          size="icon"
          aria-label="Save"
        ...
        <Button variant="ghost" size="icon" aria-label="Cancel" onClick={onCancel} data-testid={`button-cancel-edit-card-${card.id}`}>
  ...
  return (
    <Card data-testid="wizard-step-review">
      ...
      <CardContent className="space-y-4">
        ...
        <ReviewRow label="Service" value={tier ? `${tier.label} (${formatPence(tier.pricePerCardPence)})` : "Not set"} onEdit={() => props.onEditStep(1)} />
        <ReviewRow label="Cards" value={`${props.cards.length} card${props.cards.length === 1 ? "" : "s"}`} onEdit={() => props.onEditStep(2)} />

        <p className="text-xs text-muted-foreground">
          Intake observations recorded on cards are not MintVault grades. Estimated pricing is confirmed by
          MintVault.
        </p>
      </CardContent>
    </Card>
...
function ReviewRow({ label, value, onEdit }: { label: string; value: string; onEdit: () => void }) {
  return (
    <div className="flex items-center justify-between border-b pb-2" data-testid={`review-row-${label.toLowerCase().replace(/\s+/g, "-")}`}>
      <div>
        <p className="text-xs text-muted-foreground">{label}</p>
        <p className="text-sm">{value}</p>
      </div>
      <Button variant="ghost" size="sm" onClick={onEdit} data-testid={`button-review-edit-${label.toLowerCase().replace(/\s+/g, "-")}`}>
        Edit
      </Button>
    </div>
  );
  ...
  return (
    <Card data-testid="wizard-step-submit">
      ...
      <CardContent className="space-y-4">
        ...
        {missing.length > 0 && (
          <div className="rounded-md border border-destructive/30 bg-destructive/5 p-3" data-testid="text-submit-missing-info">
...
function SubmitSuccessPanel({
  ...
  return (
    <Card data-testid="wizard-submit-success">
      ...
      <CardContent className="space-y-4">
        <p>
          Reference: <span className="font-mono font-medium" data-testid="text-submission-reference">{submission.publicRef}</span>
        </p>
        <Badge data-testid="badge-submitted">Submitted to MintVault</Badge>
        <div className="flex flex-wrap gap-2 pt-2">
          <Button onClick={() => navigate(`/partner/submissions/${submission.id}`)} data-testid="button-view-submission">
            View Submission
          </Button>
