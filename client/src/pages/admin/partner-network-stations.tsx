...
function allowedActions(status: string): Array<Pick<StationAction, "label" | "route">> {
  if (status === "PENDING")
    return [
      { label: "Approve", route: "active" },
      { label: "Reject", route: "reject" },
    ];
  if (status === "ACTIVE")
    return [
      { label: "Suspend", route: "suspended" },
      { label: "Revoke", route: "revoked" },
    ];
  if (status === "SUSPENDED")
    return [
      { label: "Activate", route: "active" },
      { label: "Revoke", route: "revoked" },
    ];
  return [];
}
...
export default function PartnerNetworkStationsPage() {
  ...
  return (
    ...
    >
      {partnerId && (
        <nav aria-label="Partner workspace" className="mb-3 flex flex-wrap gap-2" data-testid="pn-workspace-tabs">
          {WORKSPACE_LINKS.map(([label, suffix]) => (
            <Link
              key={label}
              href={`/admin/partners/${partnerId}${suffix}`}
              className={`admin-chip ${suffix === "/stations" ? "is-on" : ""}`.trim()}
            >
              {label}
            </Link>
          ))}
      ...
      <Panel
        title={partnerId ? "Partner Stations" : "Network Stations"}
        sub={
          partnerId
            ? "Partner-scoped station context. Lifecycle actions use the existing step-up-protected server authority."
            : "Read-only fleet view. Open a Partner workspace for lifecycle operations."
        }
      >
        {partnerId && !validPartnerId ? (
          <div role="alert">Partner not found.</div>
        ) : (
          <>
            <div className="mb-3 flex flex-wrap gap-2">
              {statuses.map((candidate) => (
                <Chip
                  ...
                  testId={`pn-station-filter-${candidate || "all"}`}
                >
                  {candidate || "All"}
                </Chip>
              ))}
              <label className="ml-auto flex items-center gap-2 text-sm">
                ...
                <input
                  ...
                  className="ss-input"
                  aria-label="Search network stations"
                />
              </label>
            </div>
            {fleet.isLoading ? (
              <div role="status">Loading stations…</div>
            ) : fleet.isError ? (
              <div role="alert">Stations could not be loaded.</div>
            ) : (
                ...
                <tbody>
                  {(fleet.data?.stations ?? []).map((station) => (
                    <tr key={station.stationCode} id={`station-${station.stationCode}`}>
                      ...
                      <td>
                        <Link
                          href={`/admin/partners/${station.tenantId}/stations#station-${encodeURIComponent(station.stationCode)}`}
                          className="underline"
                        >
                          {station.partnerName}
                        </Link>
                      ...
                      <td>
                        <Badge variant={stationBadge(station.status)}>{station.status}</Badge>
                      </td>
                      ...
                      {partnerId && (
                        <td>
                          <div className="flex flex-wrap gap-1">
                            {allowedActions(station.status).map((action) => (
                              <AdminButton
                                key={action.route}
                                size="sm"
                                variant={action.route === "revoked" || action.route === "reject" ? "gold" : "ghost"}
                                onClick={() => {
                                  setReason("");
                                  setStationAction({ ...action, stationCode: station.stationCode });
                                }}
                                data-testid={`pn-station-${station.stationCode}-${action.route}`}
                              >
                                {action.label}
                              </AdminButton>
                            ))}
      ...
      {stationAction && (
        <div
          role="dialog"
          aria-modal="true"
          aria-labelledby="pn-station-action-title"
          className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4"
        >
          <div className="w-full max-w-md rounded bg-slate-950 p-5 shadow-xl">
            <h2 id="pn-station-action-title" className="mb-2 text-lg font-semibold">
              {stationAction.label} {stationAction.stationCode}
            </h2>
            <p className="mb-3 text-sm opacity-80">
              This uses the existing Super Admin step-up flow and records the reason in the existing station event
              trail.
            </p>
            ...
            {stationMutation.isError && (
              <div role="alert" className="mt-2 text-sm text-red-300">
                Station action could not be completed.
              </div>
            )}
            <div className="mt-4 flex justify-end gap-2">
              ...
              <AdminButton
                size="sm"
                variant="gold"
                disabled={stationMutation.isPending || !reason.trim()}
                onClick={() => stationMutation.mutate(stationAction)}
              >
                {stationMutation.isPending ? "Submitting…" : stationAction.label}
              </AdminButton>
            </div>
          </div>
        </div>
