...
import { alertBadgeVariant, formatCount, formatCredits, relativeTime, riskBadgeVariant, riskLabel, statusBadgeVariant } from "./partner-dashboard-helpers";
...
function PartnerRow({ partner }: { partner: PartnerTableRow }) {
  return <tr style={{ borderTop: "1px solid rgba(255,255,255,.08)" }}>
    <td><Link href={`/admin/partners/${partner.partnerId}`} className="underline">{partner.shopName}</Link><div className="text-xs opacity-60">{partner.publicRef}</div></td>
    ...
    <td><Link href={`/admin/partners/${partner.partnerId}/activity`} className="underline">{relativeTime(partner.lastActivityAt)}</Link></td>
  </tr>;
}
...
export default function PartnerNetworkOverviewPage() {
  ...
  useEffect(() => {
    let live = true;
    fetch("/api/admin/session", { credentials: "include" }).then((r) => r.ok ? r.json() : null).then((d) => live && setAuthed(d?.authenticated === true)).catch(() => live && setAuthed(false));
    return () => { live = false; };
  }, []);
  useEffect(() => { if (authed === false) navigate("/admin/login?next=/admin/partners", { replace: true }); }, [authed, navigate]);
  const network = useQuery<{ overview: PartnerNetworkOverview }>({
    queryKey: [BASE], queryFn: () => apiRequest("GET", BASE).then((r) => r.json()), enabled: authed === true, refetchInterval: 60_000,
  });
  const overview = network.data?.overview;
  const partners = useMemo(() => overview?.partners.rows ?? [], [overview]);
  if (authed === null || network.isLoading) return <div className="admin-root grid min-h-[60vh] place-items-center">Loading Partner Network…</div>;
  return <AdminShell activeTab="dashboard" onTabChange={() => navigate("/admin")} onLogout={() => navigate("/admin")} title="Partner Network" crumb="Overview">
    {network.isError ? <Panel title="Partner Network unavailable"><div role="alert">The consolidated Partner Network overview could not be loaded.</div></Panel> : overview && <>
      <Panel title="Network overview" sub="Authoritative consolidated projection; unavailable metrics are shown as —, never as zero.">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <StatCard label="Open corrections" value={formatCount(overview.summary.corrections.openEscalations)} mono />
          <StatCard label="Security alerts" value={formatCount(overview.summary.security.openAlerts)} mono />
          <StatCard label="Available credits" value={metricValue(overview.summary.credits.totalAvailable, formatCredits)} mono />
          <StatCard label="Reserved credits" value={metricValue(overview.summary.credits.totalReserved, formatCredits)} mono />
        </div>
      </Panel>
