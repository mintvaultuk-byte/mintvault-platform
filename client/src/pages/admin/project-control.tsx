...
export default function ProjectControlPage() {
  ...
  if (overview.isLoading || shopLaunch.isLoading) return <div className="p-8" data-testid="pc-loading" style={{ color: "var(--admin-gold)" }}>Loading Project Control…</div>;
  if (overview.isError || shopLaunch.isError || !overview.data || !shopLaunch.data) return <div className="p-8" data-testid="pc-error"><Panel title="Project Control could not load" sub="The programme remains unavailable until its authorised service responds."><p>Try again shortly. This message does not diagnose a migration or expose a backend error.</p></Panel></div>;
  ...
  return <AdminShell activeTab="dashboard" onTabChange={() => navigate("/admin")} onLogout={() => navigate("/admin")} title="Project Control" crumb="Engineering programme"><main data-testid="pc-root"><ProjectControlDashboard overview={overview.data} shopLaunch={shopLaunch.data} liveEvidence={liveEvidence.data} sync={sync.status ?? latestSync.data ?? null} onOpenPackage={(key) => navigate(`/admin/project-control/package/${key}`)} onRefresh={sync.refresh} refreshing={sync.isRefreshing} /></main></AdminShell>;
}
